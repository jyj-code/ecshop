...
<div class="pro_hot clearfix">
    ...
    <div class="hot_lists">
        ...
        <asp:Repeater ID="RepeaterData" runat="server">
            <ItemTemplate>
            <li>
                <a href='<%#ShopUrlOperate.shopHref(((DataRowView)Container.DataItem).Row["ProductGuid"].ToString(),((DataRowView)Container.DataItem).Row["MemberID"].ToString() )%>' class="hotimgs">
                        <asp:Image ID="Image1" ImageUrl='<%# ((DataRowView)Container.DataItem).Row["ZtcImg"]%>'
                            runat="server"  width="92" height="92" 
                            onerror="javascript:this.src='../ImgUpload/noImage.gif'" /></a>
                <div class="hot_name">
                    <a href='<%#ShopUrlOperate.shopHref(((DataRowView)Container.DataItem).Row["ProductGuid"].ToString(),((DataRowView)Container.DataItem).Row["MemberID"].ToString() )%>' title="<%# ((DataRowView)Container.DataItem).Row["ZtcName"].ToString()%>">
