    ...
    <div class="con_content">
        <asp:Repeater ID="RepeaterData" runat="server">
            <ItemTemplate>
                ...
                <%#Eval("MemLoginID").ToString() == "" ? "" : "<ul class=\"con_gs\"> <li><a href=\"" + ShopUrlOperate.GetShopUrlNew(Eval("shopid").ToString()) + "\">" + Eval("RealName").ToString() + "</a></li></ul>"%>
            </ItemTemplate>
        </asp:Repeater>
    </div>
</div>
...
$(document).ready(function(){
    $("#agentlist .con_content").find("h5[areacode]").each(function(m){
            ...
            var newahcode=ah5code;
            if(newahcode.indexOf(",")!=-1)
                newahcode=ah5code.substring(ah5code.indexOf(",")+1,ah5code.lastIndexOf(","));
            if(newahcode.length>=6)
                newahcode=ah5code.substring(0,6);
