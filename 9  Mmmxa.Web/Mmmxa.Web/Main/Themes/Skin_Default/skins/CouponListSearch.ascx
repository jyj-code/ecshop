...
$(function (){
   ...
   $(".comRange-text").html(OrdeState);
    ...
    (
        ...
        {
            //请求数据
            if($("#DivCity").text().trim()=="")
            {
                $("#DivCity").html(lodding);
                getCity("2","10",$("#DivCity"));
            }
            if($("#DivProvinces").text().trim()=="")
            {
...
<div class="precont">
    <div class="paixu hyfenl">
        <div class="dzyhq fl">优惠券</div>
        <div class="fr">
            <span class="paispan">所在地：</span>
          <div class="comArea">
                <span id="spanAddress"><asp:Label ID="lblAddress" runat="server"></asp:Label></span>
                <div id="souzaidi-list" class="souzaidi-list" style="display: none;">
                    <div class="citysearch">
                        <a value="000;所有地区" href="<%=ShopUrlOperate.RetUrlForSearch("ShopListSearch","-1","addcode")%>">所有地区</a>
                    </div>
            ...
            <div class="LocationBox fl" style="display:none;">
            ...
            <div class="comArea">
                ...
                <div class="souzaidi-list" style="display: none;">
                    ...
                    <asp:Repeater ID="RepeaterCategory" runat="server">
                        <ItemTemplate>
                            <a href='<%# ShopUrlOperate.RetUrl("CouponsList",((DataRowView)Container.DataItem).Row["id"].ToString()+"&addcode="+Page.Request.QueryString["addcode"]+"&catename="+((DataRowView)Container.DataItem).Row["Name"].ToString(),"category")%>' title='<%#Eval("Name")%>'>
            ...
            <div class="comRange" style=" display:none">
                ...
                <div class="moren-list" style="display: none;">
                    <a href="<%=ShopUrlOperate.RetUrlForSearch("CouponsList","browsercount&sdesc=asc","order")%>"><i class="mr-ico-pu"></i>人气从低到高</a> 
                    <a href="<%=ShopUrlOperate.RetUrlForSearch("CouponsList","browsercount&sdesc=desc","order")%>"><i class="mr-ico-pd"></i>人气从高到低</a> 
                    <a href="<%=ShopUrlOperate.RetUrlForSearch("CouponsList","createtime&sdesc=desc","order")%>">恢复默认排序</a>
                </div>
            </div>
                    ...
                    <ul>
                        <asp:Repeater ID="RepeaterData" runat="server">
                            <ItemTemplate>
                                <li>
                                    <div class="prelist">
                                        <a href='<%# ShopUrlOperate.shopDetailHrefByShopIDNew(Eval("Guid").ToString(),Eval("shopID").ToString(),"Coupon") %>'
                                            class="preimgs">
                                        ...
                                        <p>人气：<b><%#((DataRowView)Container.DataItem).Row["BrowserCount"]%></b></p>
                                    </div>
                                </li>
                            </ItemTemplate>
                        </asp:Repeater>
