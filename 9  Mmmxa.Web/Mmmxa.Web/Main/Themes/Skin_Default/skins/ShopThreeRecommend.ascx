            ...
            <div class="ranking_list cle">
                ...
                <div class="ranking_detail fr">
                    ...
                    <span class="blue1" style="white-space: nowrap">宝贝：<asp:Repeater ID="RepeaterProduct"
                        runat="server">
                        <ItemTemplate>
                            <a title='<%#((DataRowView)Container.DataItem).Row["Name"]%>' href='<%#ShopUrlOperate.shopHref(((DataRowView)Container.DataItem).Row["Guid"].ToString(),((DataRowView)Container.DataItem).Row["MemLoginID"].ToString() )%>' target="_blank" title='<%#((DataRowView)Container.DataItem).Row["Name"]%>'>
