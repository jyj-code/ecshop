...
var templateSrc = "/main/Themes/Skin_Default/"; //设置loading.gif路径
function tipsWindown(title,content,width,height,drag,time,showbg,cssName,backcall) {
	...
	switch(contentType) {
		...
		$("#windown-content").ajaxStart(function() {
			$(this).html("<img src='"+templateSrc+"/images/lodding.gif' class='loading' />");
		});
		$.ajax({
			error:function(){
	...
	if( height >= 527 ) {
		...
		$("#windown-title").css({width:(parseInt(width)+6)+"px"});
	...
	if ( _version == 6.0 ) {
		...
		$("#windown-box").css({left:"50%",top:"17%",marginTop:-((parseInt(height)+53)/2)+"px",marginLeft:-((parseInt(width)+32)/2)+"px",zIndex: "999999"});
	};
