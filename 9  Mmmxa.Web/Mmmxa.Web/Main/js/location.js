	...
	$.LocationSelect = {
		...
		build : function(user_opts) {
			...
			return $(this).each(function() {
				...
				specs.holder.append('<select name="province" class="selectarea" autocomplete="off" style=\"margin-right:5px;\"><option value="0">请选择省市</option></select><select name="city" class="selectarea"  autocomplete="off" style=\"margin-right:5px;width:105px;\"><option value="0">请选择城市</option></select><select name="district" class="selectarea"  autocomplete="off" style=\"margin-right:5px;width:105px;\"><option value="0">请选择区域</option></select>');
				try{
		...
		init: function(specs){
		    ...
		    $.each(Json_vj,function(i){
			    ...
			    $(specs.selectors.province).change(function(evt){
			        ...
			        }else{fetch(evt.target);}specs.selectors.district.html('<option value="0">请选择区域</option>');
			    });
			    specs.selectors.province.html("<option value=\"0\">请选择省市</option>");   
