...
$(function()
{
    ...
    {
        $("#tablePayment tr").each(function(){
                if($(this).find("td:eq(1)").text().indexOf("货到付款")!=-1||$(this).find("td:eq(1)").text().indexOf("线下支付")!=-1)
                {
                    $(this).hide();
                }
        });
    ...
    $(".contant").click(function()
    {
        ...
        var buynum=$("#ShoppingCart2_New_ctl00_RepeaterShopProduct_ctl00_lblBuyNumber").text();
           ...
           if(json!=null)
             {
                ...
                {
                    var price= parseFloat(json[j].fee)+parseFloat(parseInt(buynum)*parseFloat(json[j].oneadd));
                   //平邮
                   if(json[j].feetype==1)
                   {
                     $("<option value='"+price+":1"+"' lang='"+price+"'>平邮"+ price+"元</option>").appendTo("#ShoppingCart2_New_ctl00_DropDownListPost");
                   }
                   //快递
                   else if(json[j].feetype==2)
                  {
                    $("<option value='"+price+":2"+"' lang='"+price+"'>快递"+price+"元</option>").appendTo("#ShoppingCart2_New_ctl00_DropDownListPost");
                  }
                  //ems
                  else if(json[j].feetype==3)
                  {
                    $("<option value='"+price+":3"+"' lang='"+price+"'>EMS"+price+"元</option>").appendTo("#ShoppingCart2_New_ctl00_DropDownListPost");
                  }
                  else
                  {
                 ...
                 //alert($("#ShoppingCart2_New_ctl00_DropDownListPost").find("option:selected").val());
                suanqian();
             } else
                  {
                    $("<option value='0:-2'>免运费</option>").appendTo("#ShoppingCart2_New_ctl00_DropDownListPost");
         ...
         $.getJSON("/Main/Api/shopfeetemplate.ashx",params,function(json){
             if(json!=null)
             {
                    ...
                    if(json[0].Post!=0)
                    {
                        bflag=false;
                        //平邮
                        $("<option value='"+json[0].Post+":1"+"' lang='"+json[0].Post+"'>平邮"+json[0].Post+"元</option>").appendTo("#ShoppingCart2_New_ctl00_RepeaterShopCart2_ctl00_DropDownListPost");
                    }
                    if(json[0].Express!=0)
                    {
                        bflag=false;
                        //快递
                        $("<option value='"+json[0].Express+":2"+"' lang='"+json[0].Express+"'>快递"+json[0].Express+"元</option>").appendTo("#ShoppingCart2_New_ctl00_RepeaterShopCart2_ctl00_DropDownListPost");
                    }
                    if(json[0].Ems!=0)
                    {
                        bflag=false;
                        //ems
                        $("<option value='"+json[0].Ems+":3"+"' lang='"+json[0].Ems+"'>EMS"+json[0].Ems+"元</option>").appendTo("#ShoppingCart2_New_ctl00_RepeaterShopCart2_ctl00_DropDownListPost");
                    }
                    if(bflag)
                    {
             ...
             else
              {
               ...
               $("#ShoppingCart2_New_ctl00_HiddenFieldDispatchPrice").val($("#ShoppingCart2_New_ctl00_RepeaterShopCart2_ctl00_DropDownListPost").find("option:selected").attr("lang"));
         });
        }
...
/*    <!--收货人信息-->*/
  ...
  $(function(){
        ...
        function checkInput(obj)
        {
            ...
            var Regex_Mobile=/^1[3|5|8|4][0-9]\d{4,8}$/;      
            ...
            if(txtemail!=""&&!Regex_Email.test(txtemail))
            {
                $("#errormsg").show().text("邮箱格式不正确！");$(TextBoxEmail).focus();return false;
            }
             if(txtpostcode.length!=6&&txtpostcode!=""){$("#errormsg").text("邮政编码格式不对！");$(TextBoxPostalcode).focus();return false;}
//            if(txtpostcode==""){$("#errormsg").text("邮政编码不能为空！");$(TextBoxPostalcode).focus();  return false;}
            ...
            else{$(obj).hide();$(".append_image").show();
            return true;}
        }clearInput();
//        function clearInput()
//        {
