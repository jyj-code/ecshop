<%@ page language="C#" autoeventwireup="true" inherits="ShopNum1_ShopNum1Admin_ShopNum1_ProductCategory_List, ShopNum1.Deploy" %>
    ...
    <form id="form1" runat="server">
     <div id="right">
        ...
        <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                <td style=" border:none;"><a  href="ShopNum1_ProductCategory_Operate.aspx?guid=0&tc=true&width=810&height=500" class="tianjiafl lmf_btn dialog"><span>添加目录</span></a></td>
                <td>(友情提示：如果父级分类下面有子分类或者父级分类下面有商品，则该父级分类删除按钮隐藏。)</td>
                <td style="border:none;padding-left:10px; display:none;"><a href="#" class="guanlikj lmf_btn cporder"><span>批量保存</span></a></td>
                </tr>
                </table>
        </div>
            <table cellpadding="0" cellspacing="0" border="0" width="98%" class="gridview_m"
                      ...
                      {%>
                           <tr class="one lmf_huaguo" id='parent_<%=dataRow["ID"]%>'>
                              <td align="left" valign="middle">
                                ...
                                <%} %>
                                </div><div class="lmf_xuanze fl" style="display:none"><input type="checkbox" class="checkboxsn vcheck" onclick="checksub(this,'<%=dataRow["ID"]%>')" value='<%=dataRow["ID"]%>'/></div>
                                <div class="lmf_paixu fl"><input type="text" name="orderby" style="width:60px;" maxlength="8" onkeyup="NumTxt_Int(this)" lang='<%=dataRow["ID"]%>' value='<%=dataRow["OrderID"]%>' /></div>
                                <div class="lmf_titlee fl"><input type="text" value='<%=dataRow["Name"]%>' /></div>
                            </td>
                            ...
                            <td align="center" valign="middle" width="300px">
                                ...
                                <a href="ShopNum1_ProductCategory_Operate.aspx?op=edit&guid=<%=dataRow["ID"]%>&tc=true&width=810&height=500" class="dialog" style="color: #4482b4;">编辑</a>&nbsp;
                                <%if (dataRow["v"].ToString() == "" && dataRow["m"].ToString() == "")
                                  { %>
                                <a href="#" style="color: #4482b4;" lang='<%=dataRow["ID"]%>' class="delv" onclick="treelist_delete(this)">删除</a>&nbsp;
                                <%}%>
                                <a href='ShopNum1_ProductCategory_Operate.aspx?op=add&guid=<%=dataRow["ID"]%>&level=1&width=810&height=500' class="dialog" style="color: #4482b4;">添加子分类</a>
                            </td>
                          </tr>
                ...
                <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                <td style="background:#e8e8e8; border:none;padding-left:10px;">                <a  href="ShopNum1_ProductCategory_Operate.aspx?guid=0&width=810&height=500" class="tianjiafl lmf_btn dialog"><span>添加目录</span></a>(友情提示：如果分类下面有子分类或者该分类下面有商品，删除按钮隐藏。)</td>
    ...
    function NumTxt_Int(o)
{
   o.value=o.value.replace(/\D/g,'');
      ...
      function treelist_two_html(arry){
        ...
        if(arry.v!=""){xhtml+='<img style="border-width: 0px;" src="images/lmf_jia.jpg"/>';
        ...
        if(arry.isshow==1){xhtml+='<img style="border-width: 0px;" src="images/dui.jpg" onclick="treelist_isshow(this)" class="isshow" lang="'+arry.id+'-'+arry.isshow+'">';}
        else{xhtml+='<img style="border-width: 0px;" src="images/cuo.jpg" onclick="treelist_isshow(this)" class="isshow" lang="'+arry.id+'-'+arry.isshow+'">';}
        xhtml+='</td>';
    ...
    function treelist_three_html(arry)
    {
        ...
        xhtml+='<input type="text" value="'+arry.name+'" /></div></td>';
        xhtml+='<td align="center" valign="top" width="100px">';
        if(arry.isshow=="1"){xhtml+='<img style="border-width: 0px;" onclick="treelist_isshow(this)" lang="'+arry.id+'-'+arry.isshow+'" class="isshow" src="images/dui.jpg">';}
        else{xhtml+='<img style="border-width: 0px;" class="isshow" onclick="treelist_isshow(this)" lang="'+arry.id+'-'+arry.isshow+'" src="images/cuo.jpg">';}
...
$(".lmf_jia").click(function(){
    if($(this).parents(".lmf_huaguo").next("tr").is(":visible")){$(this).children("img").attr("src","images/lmf_jia.jpg");
    $(this).parents(".lmf_huaguo").next("tr").hide();}
    else
    {
        $(this).children("img").attr("src","images/lmf_jian.jpg");
        $(this).parents(".lmf_huaguo").next("tr").show();$(this).parents(".lmf_huaguo").find(".sublmf_huaguo").next("tr").hide();
    }
});
//更改子菜单展开收缩图标
...
//        }
         ...
         if(confirm("是否确定删除？")){
                   ...
                   $.get(ajaxurl+"?delectid="+$this.attr("lang"),null,function(data){
                      if(data.split("|")[0]!="ok"){
                           ...
                           for(var i in arr){$("#delete_this_"+arr[i]).remove();}   window.location.reload();
                           alert("删除成功,部分分类不能删除！");
                      }else{
    ...
    function treelist_isshow(o)
    {
       var id=$(o).attr("lang").split('-')[1];
              ...
              if($(o).attr("src")=="images/dui.jpg"){$(o).attr("src","images/cuo.jpg");$(o).attr("lang",$(o).attr("lang").split('-')[0]+"-"+id);}
              else{$(o).attr("src","images/dui.jpg");$(o).attr("lang",$(o).attr("lang").split('-')[0]+"-"+id);}
    }
    //子菜单全选反选操作
    ...
    $(function(){
         ...
         $(".cporder").click(function(){
           ...
           if(append.join(",")==""){alert("请勾选一条你要保存的数据！");return false;}
                  $.get(ajaxurl+"?vd="+escape(append.join(",")),null,function(data){});
                  alert("批量保存成功！");return false;
         });
         //背景特效
