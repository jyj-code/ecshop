<%@ page language="C#" autoeventwireup="true" inherits="Admin_SpecificationOperate"   CodeBehind="SpecificationOperate.aspx.cs"      %>
...
<head runat="server">
    ...
    <style type="text/css">
    .error{ border:1px solid red;height:22px; line-height:22px; width:130px; background:#0080ff; color:#fff;}
    </style>
    ...
    function addNewRow()
{
    ...
    {	
          ...
          spec_html.push('<tr><td align="left"><span style="color:Red;">*</span><input onkeyup="NumTxt_Int(this)" maxlength="3" ');
          spec_html.push('onafterpaste="NumTxt_Int(this)"  type=text maxlength=5 name="orderSpec_Order" class="input1" value="" /></td>');
          spec_html.push('<td align="left"><span style="color:Red;">*</span><input type=text  maxlength=20 name="specificationValue" class="input1" value="" /></td>');
          spec_html.push('<td style="width:500px;height:35px;line-height:35px;display:none;" name="tdpicshow"><div ID="divt"><a href="javascript:void(0)" onclick="SelectImage(this)"><span style="color:Red;">*</span><input type=text size=20 maxlength=20 name="picUrl"  class="input1" value="" /> <img name="imgurl"  width="30" height="30" />选择图片</a></div></td>');
    ...
    else
    {
          ...
          spec_html.push('onafterpaste="NumTxt_Int(this)"  type=text maxlength=5 name="orderSpec_Order" class="input1" value="" /></td>');
          spec_html.push('<td align="left"><span style="color:Red;">*</span><input type=text  maxlength=20 name="specificationValue" class="input1" value="" /></td>');
          spec_html.push('<td style="width:500px;height:35px;line-height:35px;" name="tdpicshow"><div><a href="javascript:void(0)" onclick="SelectImage(this)"><span style="color:Red;">*</span><input type=text size=20 maxlength=20 name="picUrl"  class="input1" value=""   /> <img name="imgurl"  width="30" height="30" />选择图片</a></div></td>');
          spec_html.push('<td align="left"><a href="javascript:void(0)" onclick="removeRow(this,0)">删除</a><input type="hidden" name="hidDG" value="0"/></td></tr>');
          $("#filearea").append(spec_html.join(""));
    }
}
...
function GetInputValues()
{
   ...
   if(document.getElementById("TextBoxSpecificationName").value==""){alert("规格名称不能为空！");document.getElementById("TextBoxSpecificationName").focus(); return false;}
   ...
   { 
        ...
        {
            if(inputs[i].name=="orderSpec_Order"){alert("排序编号不能为空！");inputs[i].focus();inputs[i].className='error';  return false;}
            if(inputs[i].name=="specificationValue"){alert("规格值不能为空！");inputs[i].focus();inputs[i].className='error'; return false;}
            if(inputs[i].name=="picUrl"&&!radioButton.checked){alert("规格图片不能为空！");inputs[i].focus();inputs[i].className='error'; return false;}
        }
   } 
   for(var i =0;i<inputs.length;i++)
   { 
        if(inputs[i].type == "text"&&inputs[i].parentNode.style.display!="none"&&inputs[i].value!="")
        {
            if(inputs[i].name=="orderSpec_Order"){arrystr.push(inputs[i].value+",");}
            if(inputs[i].name=="specificationValue"&&radioButton.checked){arrystr.push(inputs[i].value+",");}
            else if(inputs[i].name!="picUrl"&&inputs[i].name!="orderSpec_Order"){arrystr.push(inputs[i].value+",");}
            if(inputs[i].name=="picUrl"&&!radioButton.checked){arrystr.push(inputs[i].value+",");}
        }
        if(inputs[i].name=="hidDG"&&inputs[i].type == "hidden"){arrystr.push(inputs[i].value+"|");}
   } 
             ...
             <table border="0" cellpadding="0" cellspacing="1">
                <tr>
                    ...
                    <td valign="middle" colspan="2">
                            <asp:TextBox ID="TextBoxSpecificationName" runat="server" CssClass="tinput" MaxLength="4"></asp:TextBox>
                            ...
                            <asp:RegularExpressionValidator ID="RegularExpressionValidatorTitle" runat="server"
                                ControlToValidate="TextBoxSpecificationName" Display="Dynamic" ErrorMessage="最多4个字符"
                                ValidationExpression="^[\s\S]{0,4}$"></asp:RegularExpressionValidator>
                    </td>
                </tr>
                <tr>
                    <th align="right">
                    ...
                    <td align="left" colspan="2">
                            <asp:TextBox ID="TextBoxOrderID" runat="server" MaxLength="5" CssClass="tinput"></asp:TextBox>
                            ...
                            <asp:RangeValidator ID="RangeValidatorSort" runat="server" ErrorMessage="格式不正确" Display="Dynamic"
                                MaximumValue="100000" MinimumValue="1" Type="Integer" ControlToValidate="TextBoxOrderID"></asp:RangeValidator>
                    </td>
                </tr>
                 <tr class="radiobtn">
                    <th align="right">
                ...
                <tr>
                    ...
                    <td style="height: 115px;" colspan="2">
                            <asp:TextBox ID="TextBoxMemo" TextMode="MultiLine" runat="server" CssClass="tinput txtarea" MaxLength="200"></asp:TextBox>
                    </td>
                </tr>
                <tr><th></th>
                    <td colspan="2">
                ...
                <th></th>
                    <td colspan="2">
                        ...
                        <tr>
                            <td style="padding-left:65px;">排序</td>
                            <td style="padding-left:60px;">名称</td>
                            <td name="tdpicshow" style="display:none;padding-left:60px;">图片</td>
                            <td>操作</td>
                        </tr>
                        <%if (dt_Spec != null && dt_Spec.Rows.Count > 0)
                          {
                              ...
                              {%>
			                     <tr>
			                        <td align="left"><input onkeyup="NumTxt_Int(this)" maxlength="3" onafterpaste="NumTxt_Int(this)"  type=text maxlength=5 name="orderSpec_Order" class="input1" value='<%=dt_Spec.Rows[i]["oid"] %>'/></td>
			                        <td align="left"><input type=text  maxlength="20" name="specificationValue" class="input1" value='<%=dt_Spec.Rows[i]["Name"] %>' /></td>
