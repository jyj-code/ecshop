<%@ page language="C#" autoeventwireup="true" inherits="Main_Admin_Theme_ProductList"   CodeBehind="Theme_ProductList.aspx.cs"      %>
                ...
                <div class="mr">
                    <ul class="mul">
                        ...
                        <li id="current4"><a href="?state=3&Aid=<%=Common.ReqStr("aid")%>" class="<%= Common.ReqStr("state") =="3"?"cur":""%>"
                            id="LinkButtonPayment">已结束</a> </li>
                        <li id="current5"><a href="Theme_ActivityList.aspx">返回列表</a> </li>
                    </ul>
                </div>
            </div>
        </div>
            ...
            <table cellspacing="0" cellpadding="4" border="0" id="Num1GridViewShow" rules="cols"
                ...
                <tbody>
                    <%if (dt.Rows.Count > 0)
                      {
                          for (int i = 0; i < dt.Rows.Count; i++)
                          {
                              ...
                              string strProductUrl = ShopUrlOperate.RedirectProductDetailByMemloginIDNew(dt.Rows[i]["Guid"].ToString(), dt.Rows[i]["MemloginId"].ToString(), "0", "0");
                    %>
                    <!--循环代码-->
                    <tr style="cursor: default;" onmouseout="Num1GridViewShow_mout(this)" onmouseover="Num1GridViewShow_mover(this)">
                        ...
                        <td align="center">
                            <span class="opspan"><a href="<%=ShopUrlOperate.RedirectProductDetailByMemloginIDNew(strProGuId, strMemloginId,"0","0")%>"
                                target="_blank" style="color: #4482b4;">查看</a></span>|
                            <%if (strState == "未审核")
                              { %>
                            <span class="opspan oppass"><a style="color: #4482b4;" href="?agree=<%=strId %>&Aid=<%=strThemeGuid %>&jk"
                                onclick="return window.confirm('您确定要审核通过吗?');">审核通过</a></span>| <span class="opspan opupass">
                                    <a style="color: #4482b4;" href="?disagree=<%=strId %>&Aid=<%=strThemeGuid %>&jk" onclick="return window.confirm('您确定要不予通过吗?');">
                                        不予通过</a></span>|
                            <%}
