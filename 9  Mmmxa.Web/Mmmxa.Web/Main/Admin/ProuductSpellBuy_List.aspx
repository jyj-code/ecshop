<%@ page language="C#" autoeventwireup="true" inherits="AdminProuductSpellBuy_List, ShopNum1.Deploy" %>
              ...
              <%if (dt != null)
                {
			      ...
			      <tr style="cursor:default;" onmouseout="Num1GridViewShow_mout(this)" onmouseover="Num1GridViewShow_mover(this)">
			            <td align="center" style="">
                           ...
                           <a href='<%= ShopUrlOperate.shopDetailHrefByShopID(dt.Rows[i]["id"].ToString(),dt.Rows[i]["shopid"].ToString(),"ProductIsSpell_Detail") %>' target="_blank">
                            <%=ShopNum1.Common.Utils.GetUnicodeSubString(dt.Rows[i]["pname"].ToString(), 35, "")%>
                           </a>
                        </td><td align="center">
                           <a href='<%= ShopUrlOperate.shopDetailHrefByShopID(dt.Rows[i]["id"].ToString(),dt.Rows[i]["shopid"].ToString(),"ProductIsSpell_Detail") %>' target="_blank">
                           ...
                           <%=dt.Rows[i]["groupprice"]%>
                            ...
                            <a href='<%=ShopUrlOperate.GetShopUrl(dt.Rows[i]["shopid"].ToString()) %>' target="_blank"><%=dt.Rows[i]["shopNAME"]%></a>
                        </td>
                        <td align="center">
                            <%=dt.Rows[i]["starttime"] %>
                        </td>
                        ...
                        <td align="center">
                           <span onclick="if(confirm('是否删除？')){location.href='?delete=<%=dt.Rows[i]["id"] %>'}">删除</span>
                        </td>
                 </tr>
			    <!--循环代码-->
                    <%}
                ...
                %>
                    <tr><td colspan="10"><center> 没有找到相关团购商品！</center> </td></tr>
                <%} %>
