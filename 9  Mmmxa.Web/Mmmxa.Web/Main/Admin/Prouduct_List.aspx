<%@ page language="C#" autoeventwireup="true" inherits="AdminProuduct_List, ShopNum1.Deploy" %>
...
<head id="Head1" runat="server">
    ...
    return this.replace(/(^\s*)|(\s*$)/g,'');
        ...
        function checkPrice(){$("#TextBoxPrice1").val($("#TextBoxPrice1").val().trim());$("#TextBoxPrice2").val($("#TextBoxPrice2").val().trim());
            return true;
            }
            ...
            <div class="order_edit">
                <table border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td>
                         ...
                         <td>
                            <asp:Button ID="Button1" runat="server" Text="查询" CssClass="dele" OnClientClick="return checkPrice()" OnClick="ButtonSearch_Click" /><asp:DropDownList
                                ID="DropDownListIsAudit" Visible="false" runat="server" CssClass="tselect" Style="width: 100px;">
                            </asp:DropDownList>
                        </td>
                    </tr>
                </table>
                ...
                <td valign="top">
                     <asp:DropDownList ID="DropDownListOperate" runat="server" CssClass="tselect" Width="150">
                        ...
                        <asp:ListItem Value="IsNew,1">设为新品</asp:ListItem>
                        <asp:ListItem Value="IsSystemHot,1">设为主站热卖</asp:ListItem>
                        <asp:ListItem Value="IsShopGood,1">设为精品</asp:ListItem>
                        <asp:ListItem Value="IsSystemRecommend,1">设为主站推荐</asp:ListItem>
                        ...
                        <asp:ListItem Value="IsSystemHot,0">取消主站热卖</asp:ListItem>
                        <asp:ListItem Value="IsGood,0">取消精品</asp:ListItem>
                        <asp:ListItem Value="IsSystemRecommend,0">取消主站推荐</asp:ListItem>
                          <asp:ListItem Value="IsSaled,0">下架</asp:ListItem>
                       </asp:DropDownList>
                ...
                <td valign="top" class="lmf_app">
                    <asp:LinkButton ID="ButtonChangeOrderID" runat="server"  CssClass="glpx lmf_btn"
                        OnClientClick="return EditButton()" OnClick="ButtonChangeOrderID_Click"   ><span>管理排序</span></asp:LinkButton>
             ...
             <table cellspacing="0" cellpadding="4" border="0" id="Table1" rules="cols" class="gridview_m">
                 ...
                 <th scope="col">城市分站</th><th scope="col">商品名称</th><th scope="col">店铺名</th><th scope="col">商品图片</th><th scope="col">商品价格</th>
                 <th scope="col">库存</th><th scope="col">所属分类</th><th scope="col">新品</th><th scope="col">主站热卖</th>
                 <th scope="col">主站推荐</th><th scope="col">可售</th><th scope="col">上架</th><th scope="col">排序号</th><th scope="col">操作</th>
          </tr>
          </thead>
          <tbody>
                  ...
                  for (int i = 0; i < dt_ProductList.Rows.Count; i++)
                  {
                      DataRow dr = dt_ProductList.Rows[i];%>
                       <tr style="cursor:default;" onmouseout="Num1GridViewShow_mout(this)" onmouseover="Num1GridViewShow_mover(this)">
                                ...
                                <td align="center"><%=GetSubstationIDname(dr["strSubstationID"].ToString())%></td>
                                <td align="center"><%=dr["Name"].ToString().Length > 16 ? dr["Name"].ToString().Substring(0, 16) : dr["Name"].ToString()%></td>
                                <td align="center"><%=dr["ShopName"] %></td>
                                <td align="center"> <img src='<%=Page.ResolveUrl(dr["OriginalImage"].ToString())%> ' onmouseover="javascript:ddrivetip('<img width=200px height=200 src=<%=Page.ResolveUrl(dr["OriginalImage"].ToString())%> >','#ffffff')"
                                onmouseout="hideddrivetip()" height="20" width="20" /></td>
                                <td align="center"><%=dr["ShopPrice"] %></td>
                                <td align="center"><%=dr["RepertoryCount"]%></td>
                                <td align="center"> <%=ShopNum1.Common.Utils.GetUnicodeSubString(dr["ProductCategoryName"].ToString(), 25, "")%></td>
                                <td align="center"><img src='<%=PageOperator.GetListImageStatus(dr["IsNew"].ToString()=="0"?"1":"0")%>' /></td>
                                <td align="center"><img src='<%=PageOperator.GetListImageStatus(dr["IsHot"].ToString()=="0"?"1":"0")%>' /></td>
                                <td align="center"><img src='<%=PageOperator.GetListImageStatus(dr["IsRecommend"].ToString()=="0"?"1":"0")%>' /></td>
                                <td align="center"><img src='<%=PageOperator.GetListImageStatus(dr["IsSell"].ToString()=="0"?"1":"0")%>' /></td>
                                <td align="center"><img src='<%=PageOperator.GetListImageStatus(dr["IsSaled"].ToString() == "0" ? "1" : "0")%>' /></td>
                                <td align="center"><%=dr["SystemOrderID"]%></td>
                                <td align="center"> <a href="ProductSearchDetal.aspx?guid=<%=dr["guid"] %>&Type=Other&Back=1" style="color: #4482b4;">
                                查看</a>&nbsp;&nbsp;<a href="javascript:void(0)" onclick="if(confirm('是否删除该商品?')){window.location.href='Prouduct_List.aspx?guid=<%=dr["guid"] %>&sign=del';}" style="color: #4482b4;">删除</a></td>
                        </tr>
                  <%}
              } else
                  { %><tr><td height="30" align="center" colspan="15"><strong style="font-size:13px;">无查询的记录！</strong></td></tr><%} %>
              </tbody>
            </table>
              <div class="btnlist" style="overflow:auto;" id="showPage" runat="server" visible="false">
		       <div class="fnum"> 每页显示数量：
		         <a href="?pagesize=10" id="page10">10</a>
		         <a href="?pagesize=20" id="page20">20</a>
		         <a href="?pagesize=50" id="page50">50</a>
		       </div>
                        ...
                        function checknum(str)
                        {
                            ...
                            if (!re.test(str))
                            {
		                        ...
		                        $(function(){ 
		                              $("input[name='vjpage']").keyup(function(event){
		                                     var pagesize='<%=ShopNum1.Common.Common.ReqStr("pagesize") %>';
		                                     if(event.keyCode==13){ location.href='?pagesize='+pagesize+'&pageid='+$("input[name='vjpage']").val();return false;}
		                               });
		                               var pagesize='<%=ShopNum1.Common.Common.ReqStr("pagesize") %>';
