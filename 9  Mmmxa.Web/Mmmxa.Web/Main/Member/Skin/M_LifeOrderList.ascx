            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="5" class="blue_tbw" id="ord_<%=i%>">
                <tr>
                    <th scope="col" colspan="5" class="th_le">
                        ...
                        商家：<%= Common.cut(dr["shopname"],20)%>&nbsp;&nbsp;&nbsp;<strong>下单时间：<%=dr["createtime"]%></strong>
                    </th>
                    <th scope="col" class="th_color th_ri">
                        订单状态：<span><%=M_LifeOrderList.setOrderState(dr["oderstatus"].ToString(), dr["PaymentStatus"].ToString(), dr["refundStatus"].ToString())%></span>
                    </th>
                </tr>
            </table>
            <table width="100%" border="0" cellspacing="0" cellpadding="5" class="blue_tb1" style="margin-top: -1px;"
                ...
                <tr>
                    ...
                    <td rowspan="<%=drr.Length%>" style="width: 70px;">
                        ...
                        <%if (dr["ocid"].ToString() != "")
                          { %>
                       <div> <a href='m_index.aspx?tomurl=M_ComplaintsDetailed.aspx?id=<%=dr["ocid"].ToString() %>'
                            target="_blank">查看投诉</a></div>
                        <%}
                          else
                          { %>
                       <div> <a href='m_index.aspx?tomurl=M_MemberComplaints.aspx?shopid=<%=drr[k]["shopid"] %>|<%=dr["ordernumber"] %>'
                    ...
                    <td rowspan="<%=drr.Length%>">
                     <%if (Convert.ToInt32(dr["oderstatus"]) > 3)
                       { %>
                        ...
                        <%if (dr["oderstatus"].ToString() == "0" && dr["paymentname"].ToString()!="线下支付")
                          { %>
