...
<script type="text/javascript" language="javascript">
            $(function(){
                    //控制状态选中
                    var state=$("#M_OrderDetail_ctl00_hidOrderState").val();
                    for(var i=0;i<4;i++)
                    {
                    ...
                    $("#M_OrderDetail_ctl00_butEditPay").click(function(){
                            $("#dwindow").show(); 
                    });
                    //支付选中
                    $("input[name='radio_pay']").click(function(){
        $("#M_OrderDetail_ctl00_hidPayMentInfo").val($(this).val()+"|"+$(this).attr("pay_Name")+"|"+$(this).attr("pay_Charge")+"|"+$(this).attr("pay_IsPercent"));
                    });
                    
                    var hidpaystate=parseInt($("#M_OrderDetail_ctl00_hidPayState").val());
                    var hidshipstate=parseInt($("#M_OrderDetail_ctl00_hidShipState").val());
                    var hidrefundstatus=parseInt($("#M_OrderDetail_ctl00_hidRefundstatus").val());
                    var hidRefundType=parseInt($("#M_OrderDetail_ctl00_hidRefundType").val());
                    if(hidpaystate>1||hidshipstate>2||hidrefundstatus>0)
                    {
                       $("#payupdate").hide();$("#M_OrderDetail_ctl00_butSureConfirm").hide();$("#payupdate3").hide();
                    }
                    if((hidshipstate=="5"||hidshipstate=="1"||hidpaystate=="2")&&(hidrefundstatus=="1"||hidrefundstatus=="2")&&state=="2")
                    {
                        $("#payupdate").show();$("#M_OrderDetail_ctl00_butSureConfirm").show();$("#payupdate3").show();
                    }
                    if(hidrefundstatus=="1"&&state=="1"&&hidRefundType=="0")
                    {
                        $("#refundtxt").text(", 该订单正在退款...");
                        $("#M_OrderDetail_ctl00_lblOrderStateTxt").text($("#M_OrderDetail_ctl00_lblOrderStateTxt").text()+",退款中...");
                    }
                    if(hidrefundstatus=="2"&&state=="1"&&hidRefundType=="0")
                    {
                        $("#refundtxt").html(", 卖家拒绝退款,您可以点击<a target=\"_blank\" href=\"m_index.aspx?tomurl=M_MemberComplaints.aspx?shopid="+$("#M_OrderDetail_ctl00_lblMemLoginId").text()+"|"+$("#M_OrderDetail_ctl00_lblOrderNumber").text()+"\">【投诉】</a>,请求平台处理。");
                        $("#M_OrderDetail_ctl00_lblOrderStateTxt").text($("#M_OrderDetail_ctl00_lblOrderStateTxt").text()+",卖家拒绝退款.");
                    }
                    if(hidrefundstatus=="1"&&state=="2"&&hidRefundType=="1")
                    {
                        $("#vexitstate").text(", 该订单正在退货...");$("#payupdate3").show();$("#M_OrderDetail_ctl00_butSureConfirm").show();
                        $("#M_OrderDetail_ctl00_lblOrderStateTxt").text($("#M_OrderDetail_ctl00_lblOrderStateTxt").text()+",退货中...");
                    }
                    if(hidrefundstatus=="2"&&state=="2"&&hidRefundType=="1")
                    {
                        $("#vexitstate").html(", 卖家拒绝退货,您可以点击<a target=\"_blank\" href=\"m_index.aspx?tomurl=M_MemberComplaints.aspx?shopid="+$("#M_OrderDetail_ctl00_lblMemLoginId").text()+"|"+$("#M_OrderDetail_ctl00_lblOrderNumber").text()+"\">【投诉】</a>,请求平台处理。");
                         $("#M_OrderDetail_ctl00_lblOrderStateTxt").text($("#M_OrderDetail_ctl00_lblOrderStateTxt").text()+",卖家拒绝退货.");
                    }
                    if($("#M_OrderDetail_ctl00_hidlifetype").val()=="2")
                    {
                    $("#trancommpany").hide();$("#tranord").hide();$("#traninfo").hide();
                            ...
                            if(state == "3")
                            {
                                ...
                                $("#M_OrderDetail_ctl00_lblSendGoodsTime").text(""+ $("#M_OrderDetail_ctl00_lblPayMentTime").text() + " ~ " + $("#M_OrderDetail_ctl00_lblConfirmOrderTime").text() +"")
                             }
                    }
                    else
                    {
                    ...
                    var ev=$("#M_OrderDetail_ctl00_hidEndTime").val();
                    if(ev!=""){
                        show_date_time(ev.replaceAll("-","/"),"suretime");
                    }
                    
                    var edd=$("#M_OrderDetail_ctl00_hidCanleTime").val();
                    if(edd!="")
                    {
                         show_date_time(edd.replaceAll("-","/"),"canceltime");
                    }
                    if($("#M_OrderDetail_ctl00_lbllogisticType").text()=="不需要物流")
                    {
                        $("#trancommpany").hide();$("#tranord").hide();$("#traninfo").hide();
                    }
                        
//                        var strue='<%=Common.ReqStr("send") %>';
//                        if(strue=="sure")
//                        {
//                            $("#M_OrderDetail_ctl00_butSureConfirm").click();
//                        }
                        var logisticname=$("#M_OrderDetail_ctl00_lblLogisticsCompany").text();
                        var evp=$("#errordiv p").text();
                        if(evp!="")
                        {
                            var LogisticNum=$("#M_OrderDetail_ctl00_lblShipmentNumber").text();
                            ...
                            {
                                $("#errordiv p").html("您选择的物流公司尚未提供物流跟踪信息反馈。如需了解请咨询物流公司。<a href=\"/Express.html?tag=11&LogisticNum="+LogisticNum+"\" target=\"_blank\">查询快递</a>");
                            }
                        }
                        if(location.href.indexOf("toplocation")!=-1)
                        {
...
$.get("/Api/getdate.ashx?r="+Math.random(),null,function(data){
 var severtime = new Date(data.replaceAll("-","/"));//服务器时间
    ...
    document.getElementById(element).innerHTML=daysold+"天"+hrsold+"小时"+minsold+"分"+seconds+"秒" ;
            ...
            function checkReceive()
            {
                ...
                $("#M_OrderDetail_ctl00_hidShopPrice").val(xprice.join(","));
                $("#M_OrderDetail_ctl00_hidOrderProductId").val(xarrid.join(","));
                $("#M_OrderDetail_ctl00_hidBuyNum").val(xnum.join(","));
                var strMsg="点击确定后,您之前付款的 "+$("#M_OrderDetail_ctl00_lblShouldPrice").text()+" 元将直接到卖家帐户里,请务必收到货再确认!";
                if($("#M_OrderDetail_ctl00_hidRefundstatus").val()=="1")
                {
                    strMsg+="\r\n该订单处于退货中,如果确认收货将会关闭退款,是否确认收货?";
                }
                if(confirm(strMsg))
                {
                    $("#<%=butSureConfirm.ClientID %>").hide();$(".append_image").show();
                    return true;
                }
                else
                {
    ...
    function DelaySure()
    {
         if(parseInt($("#M_OrderDetail_ctl00_hidTotalDays").val())>=3)
         {
    ...
    {
        var shoploginid=$("#M_OrderDetail_ctl00_lblMemLoginId").text();
        var ordernumber=$("#M_OrderDetail_ctl00_lblOrderNumber").text();
        if($(o).attr("lang")=="1")
        {
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="order_tab">
            ...
            <tr style="height: 30px;">
                <td class="arrow_1" valign="top">
                    <div><asp:Label ID="lblSetOrderTime" runat="server"></asp:Label>&nbsp;</div>
                    <div id="payupdate0" class="payupdate" style="display:none;">
                        <div class="xiaojt">
                        </div>
                        <div class="orderxx">
                             <ul><%if (hidPayMentName.Value == "货到付款" || hidPayMentName.Value == "线下支付") { butPayMoney.Visible = false; hidshow.Visible = false; butEditPay.Visible = false; }%>
                                <li class="ordul1">当前订单状态：宝贝已拍下，请买家尽快付款</li>
                                <li>1.<span id="hidshow" runat="server">点击这里<asp:LinkButton ID="butPayMoney" runat="server" Text="付　款 " CssClass="zf_bot"/></span>   如果您在<asp:Label ID="lblCancleOrder" runat="server"></asp:Label>天内未付款,系统将在<span id="canceltime" style="color:Red"></span>后取消订单。
                                    </li>
                                <li>2.如果你想取消这笔订单，可以
                                    <asp:LinkButton ID="butCancelOrder" OnClientClick="if(confirm('是否取消订单？')){return true;}else{return false;}" runat="server" Text="取消订单" CssClass="zf_bot" /></li>
                                <li>3.您当前的支付方式为[<%=hidPayMentName.Value%>]<input type="button" id="butEditPay" runat="server" value="修改支付方式" class="zf_bot"/></li>
                            </ul>
                        </div>
                    </div>
                </td>
                <td class="arrow_2" valign="top">
                    <div><asp:Label ID="lblPayMentTime" runat="server"></asp:Label>&nbsp;</div>
                    <div id="payupdate1" class="payupdate payupdate1" style="display: none">
                        <div class="xiaojt">
                        </div>
                        <div class="orderxx">
                            <ul>
                                <li class="ordul1">当前订单状态：买家已付款<span id="refundtxt">, <span class="hidpaystate">请通知卖家尽快发货</span><span class="showlife">等待买家消费</span></span></li>
                ...
                <td class="arrow_3" valign="top">
                   <div><asp:Label ID="lblSendGoodsTime" runat="server"></asp:Label>&nbsp;</div>
                ...
                <td class="arrow_4" valign="top">
                    <div><asp:Label ID="lblConfirmOrderTime" runat="server"></asp:Label>&nbsp;</div>
                    <div id="payupdate3" class="payupdate3" style="display: none;">
                        ...
                        <div class="orderxx">
                            <ul>
                                ...
                                <li>3.如果您已经收到货，请点击 
                                   <asp:LinkButton ID="butSureConfirm" runat="server" CssClass="orang_a" OnClientClick="return checkReceive()"  Text="【确认收货】" /> <img class="append_image" alt="正在加载请稍等候" src="/Main/Themes/Skin_Default/Images/lodding.gif" style="display:none;" />(如果您在<asp:Label ID="lblSureDays" runat="server" runat="server"></asp:Label>天内未确认收货,卖家将在<span id="suretime" style="color:Red"></span>后可以执行确认收货操作!)
                                    <asp:Label ID="lblDelay" runat="server" Text="您可以延迟3天收货"></asp:Label>
                                    <%if (lblDelay.Text.IndexOf("您已延迟过")==-1)
                                      { %>
                                    ,【<asp:LinkButton ID="btnDelay" Enabled="false" runat="server" OnClientClick="return DelaySure()" Text="我要延迟收货" CssClass="zf_bot"></asp:LinkButton>】<%} %></li>
                            </ul>
                        </div>
                    </div>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                        <td style="width: 320px;">
                            掌柜ID：<asp:Label ID="lblMemLoginId" runat="server"></asp:Label><a href="M_index.aspx?tomurl=M_SendMsg.aspx?ordernumber=<%=lblOrderNumber.Text %>" target="_blank" class="fasong" style="color:#ffffff;">发送消息</a>
                            <%if (hidOrderState.Value == "1")
                              { %>
                            <span onclick="sendcheck(this)" lang="0" class="fasong" style="color:#ffffff; cursor:pointer;">提醒发货</span>
                            <%} %>
                        </td>
                        <td>
                        ...
                        <td>
                            城市：<%=lblAreaName.Text.Replace("0","")%><asp:Label ID="lblAreaName" runat="server" Visible="false"></asp:Label>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="2">
                ...
                <table width="100%" border="0" cellspacing="1" cellpadding="0" class="orderta">
                    ...
                    <tr>
                        ...
                        <td style="height: 60px;">
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td style="padding-right: 0px;width:61px; padding-left: 2px; text-align:left; border: none">
                                        <a href="<%=ShopUrlOperate.shopHrefNew(dr_Product["ProductGuId"].ToString(),dr_Product["ShopId"].ToString())%>"
                                            target="_blank">
                        ...
                        <td>
                            <strong class="red">
                                <%=Convert.ToDecimal(dr_Product["BuyPrice"])%></strong>
                        </td>
                        <%--  <td></td>--%>
                    </tr>
                    <%}
...
<!--修改支付方式弹出层-->
        <div style="position:fixed; display: none;left: 0; top: 0; width:100%; height:100%;z-index:9999;_position:absolute;_top:expression(eval(document.documentElement.scrollTop+0));"
            id="dwindow">
            <div style="display: block;" class="sp_overlay"></div>
            <div style="width:600px; position:relative;  z-index:1000; margin:0px auto 10px; top:0px; background:#c5c5c5; border-radius: 3px 3px 3px 3px; padding:6px;">
            ...
            <div class="pop_content" id="dwindowcontent" style="float:none; margin:0;">
                    ...
                    <%if (dt_pay != null)
                      {
                          for (int i = 0; i < dt_pay.Rows.Count; i++)
                          {
                              ...
                              if (dr["name"].ToString().IndexOf("货到付款") == -1)
                              {
                    %>
                    <!--支付方式循环代码-->
                ...
                <div style="margin: 4px; text-align: right; padding-bottom:15px;">
                    <asp:LinkButton ID="btnUpdatePayMent" runat="server" Text="确定" CssClass="baocbtn" style="display:block; margin:0 auto; color:#ffffff;" /></div>
            </div>
            </div>
            </div>
        </div>
