...
<script type="text/javascript" language="javascript">
     function IsDelete(val){
        if(confirm("是否删除勾选项？"))
         {
               $.get("/Api/Main/Member/DeleteOp.ashx?type=ShopNewsComment&delid="+val,null,function(data){
    ...
    <asp:Repeater ID="RepeaterShow" runat="server">
        <ItemTemplate>
            <tr>
                ...
                <td style="text-align: left;">
                    <a target="_blank" href='<%#ShopUrlOperate.shopDetailHrefByShopID(Eval("ArticleGuid").ToString(),Eval("ShopID").ToString(),"NewsDetail")%>'>
                        <%#ShopNum1.MemberWebControl.M_ShopNewsComment.GetTitle(Eval("ArticleGuid").ToString()).Length > 20 ? ShopNum1.MemberWebControl.M_ShopNewsComment.GetTitle(Eval("ArticleGuid").ToString()).Substring(0, 20) : ShopNum1.MemberWebControl.M_ShopNewsComment.GetTitle(Eval("ArticleGuid").ToString())%>&nbsp;
                    </a>
                </td>
                <td>
                    <%#Eval("CommentTime")%>&nbsp;
                ...
                <td class="th_ri">
                    <a href="M_ShopNewsCommentDetail.aspx?guid=<%#Eval("Guid")%>">查看</a>
                </td>
            </tr>
        </ItemTemplate>
    </asp:Repeater>
