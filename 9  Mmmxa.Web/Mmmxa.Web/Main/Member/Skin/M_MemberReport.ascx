...
<script  type="text/javascript">
    ...
    {
        var Member=$("#M_MemberReport_ctl00_TextBoxID").val();
        if(Member!="")
        {
            ...
            $.ajax({
                ...
                success: function (ajaxData) 
                {
                     if(ajaxData=="false")
                     {
                        ...
                        $("#M_MemberReport_ctl00_HiddenFieldMember").val("no");
                     }
                     else if(ajaxData=="true")
                     {
                        ...
                        $("#errMember").css("color","red");
                        $("#loading").css("display","none");
                        $("#M_MemberReport_ctl00_HiddenFieldMember").val("ok");
                     }
                }
                })
        }
    ...
    {
        var text=$("#M_MemberReport_ctl00_TextBoxProductUrl").val();
        if(text!="")
        {
    ...
    {
        var type=$("#M_MemberReport_ctl00_DropDownListType").val();
        if(type=="-1")
        {
    ...
    {
        var Evidence=$("#M_MemberReport_ctl00_TextBoxEvidence").val();
        if(Evidence!="")
        {
            $("#errEvidence").html("*");
    ...
    {
        funCheckMember();
        if($("#M_MemberReport_ctl00_HiddenFieldMember").val()=="ok"&& funCheckUrl()&&funCheckType()&&funCheckEvidence())
        {
            return true;
        }
        else
  ...
  <tr>
    <th colspan="2" scope="col">我要举报</th>
    </tr>
  <tr>
    <td class="bordleft">举报商家的ID号：</td>
    <td class="bordrig" >
     <asp:TextBox ID="TextBoxID" runat="server" CssClass="textwb" onblur="funCheckMember()"></asp:TextBox>
    <span class="red" id="errMember">*</span>
    <span id="loading" style=" clear:both; vertical-align:middle; display:none; float:right">
  ...
  <tr>
    <td class="bordleft">证据：</td>
    <td class="bordrig" style="padding-top:8px;">
        <asp:TextBox ID="TextBoxEvidence" runat="server" TextMode="MultiLine" onblur="funCheckEvidence()" Height="160" Width="500" CssClass="textwb"></asp:TextBox>
   <span class="red" id="errEvidence">*</span>
    </td>
  </tr>

  <tr>
    <td class="bordleft" style=" border-bottom:solid 1px #c6dfff;" >上传图片：</td>
    <td class="bordrig" style=" border-bottom:solid 1px #c6dfff;">
        <asp:FileUpload ID="FileUploadImage" runat="server" name="fileField"/>
   <%--   <span class="red">*</span>--%><span>上传图片不超过5M，上传图片支持gif、jpg、png格式。</span>
    </td>
  </tr>
</table>
