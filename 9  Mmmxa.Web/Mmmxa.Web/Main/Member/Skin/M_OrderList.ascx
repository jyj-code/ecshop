...
<% DataTable dt = M_OrderList.dt_OrderList; %>
...
<script type="text/javascript" language="javascript">
       $(function(){
                ...
                $("#sk").click(function(){
                        ...
                        var ostate='<%=Common.ReqStr("ostate") %>';
                        var sstate='<%=Common.ReqStr("sstate") %>';
                        var pstate='<%=Common.ReqStr("pstate") %>';
                        var stype='<%=Common.ReqStr("stype") %>';
                        location.href="M_OrderList.aspx?ostate="+ostate+"&sstate="+sstate+"&pstate="+pstate+"&stype="+stype+"&pn="+escape($("#pname").val())+"&ord="+$("#ordernum").val()+"&sd="+$("#txtstartdate").val()+"&ed="+$("#txtenddate").val()+"&sn="+escape($("#shopname").val())+"&st="+state+"&";
                });
                // 全选
        ...
        function ontoPage(o)
        {
           ...
           if(checknum(pageindex))
           {
                ...
                if(pageEnd<=pageindex)
                    pageindex=pageEnd;
                var stype='<%=Common.ReqStr("st")%>';
                location.href="M_OrderList.aspx?st="+stype+"&pageid="+pageindex;
           }
        }
</script>
...
<div id="list_main">
    <ul class="sidebar">
        <li class='<%=Common.ReqStr("st")=="all"||Common.ReqStr("st")==""?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=all">全部</a></li>
        <li class='<%=Common.ReqStr("st")=="0"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=0">等待付款</a></li>
        <li class='<%=Common.ReqStr("st")=="1"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=1">等待卖家发货</a></li>
        <li class='<%=Common.ReqStr("st")=="2"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=2">待确认收货</a></li>
        <li class='<%=Common.ReqStr("st")=="3"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=3">交易成功</a></li>
        <li class='<%=Common.ReqStr("st")=="4"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="M_OrderList.aspx?st=4">交易关闭</a></li>
    </ul>
    <div id="content" class="ordmain">
        <div class="pad">
            <table border="0" cellspacing="0" cellpadding="0" class="lineh">
                ...
                <tr class="up_spac">
                    ...
                    <td>
                        <input name="shopname" id="shopname" type="shopname" class="ss_nr1" value='<%=Common.ReqStr("sn") %>' />
                    </td>
                    <td>
        ...
        <div class="shangpinkjdd">
            ...
            <%if (dt != null)
              {
                  DataTable dt_Sub = M_OrderList.dt_GetOrderProduct(""); bool bflag = true;
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="5" class="blue_tbw" id="ord_<%=i%>">
                <tr>
                    ...
                    <th scope="col" class="th_color th_ri">
                        订单状态：<span><%=M_SaleOrderList.setOrderState(dr["oderstatus"].ToString(), dr["PaymentStatus"].ToString(), dr["refundStatus"].ToString())%></span>
                    </th>
                </tr>
            </table>
            <table width="100%" border="0" cellspacing="0" cellpadding="5" class="blue_tb1" style="margin-top: -1px;"
                ...
                <%
                    ...
                    {
                        string strUrl = ShopUrlOperate.shopHrefNew(drr[k]["ProductGuId"].ToString(), drr[k]["ShopId"].ToString());
                        if(dr["ordertype"].ToString()=="4")
                            strUrl = ShopUrlOperate.shopDetailHrefByShopIDNew(drr[k]["ProductGuId"].ToString(), drr[k]["ShopIds"].ToString(), "ProductIsPanic_Detail");
                ...
                <tr>
                    <td width="82">
                        <a class="alink_blue" href="<%=strUrl%>"
                    ...
                    <% }
                      ...
                      {%>手机客户端<%}   else if (dr["ordertype"].ToString() == "9")
                      {%>wap站<%}  else if (dr["ordertype"].ToString() == "10")
                      {%>微信商城<%} %>
                        <span>
                    ...
                    <td width="70">
                        <span class="redbold">
                            <%=Convert.ToDecimal(drr[k]["ShopPrice"])%></span>
                    </td>
                    <td width="30">
                        <%=drr[k]["buynumber"]%>
                    </td>
                                ...
                                <%if (dr["refundtype"].ToString() == "0")
                                  {
                                      ...
                                      <%}
                                      else if (dr["vrefund"].ToString() == "0" && Convert.ToInt32(dr["oderstatus"]) == 1)
                                      {%>
                                            <a href="m_index.aspx?tomurl=M_RefundGoods.aspx?view=exitmoney|<%=drr[k]["shopid"] %>|<%=dr["guid"] %>|@<%=dr["oderstatus"] %>#commonorder&"
                                  ...
                                  {%>
                                      <a href="m_index.aspx?tomurl=M_RefundGoods.aspx?op=exitmoney|<%=drr[k]["shopid"] %>|<%=dr["guid"] %>|@<%=dr["oderstatus"] %>#commonorder&" target="_search" class="redv">退款</a>
                                  <%}
                    ...
                    <td rowspan="<%=drr.Length%>">
                      <%if (Convert.ToInt32(dr["oderstatus"]) > 3)
                        ...
                        <%}
                          ...
                          {%>
                               <a href="M_OrderDetail.aspx?guid=<%=dr["guid"]%>&ordertype=<%=dr["ordertype"] %>&send=sure" class="alink_blue" target="_parent">确认收货</a><br />
                               <a href="M_OrderDetail.aspx?guid=<%=dr["guid"]%>&ordertype=<%=dr["ordertype"] %>&send=sure&&toplocation=show" class="alink_blue" target="_parent">查看物流</a><br />
                          <%}
                          else if (dr["oderstatus"].ToString() == "3" && dr["IsBuyComment"].ToString() == "0")
                          { %>
                            <a href="M_ProductComment.aspx?orid=<%=dr["guid"] %>" class="alink_blue" target="_parent">评价</a>
                        <%}
                          else if (dr["IsBuyComment"].ToString() == "1")
                          { %>
                            <a href="M_AddProductComment.aspx?orid=<%=dr["guid"] %>" class="alink_blue" target="_parent"> 追加评价</a>
                        <%} %>
                    </td>
                    <%} %>
                    <!--操作显示一行一列-->
