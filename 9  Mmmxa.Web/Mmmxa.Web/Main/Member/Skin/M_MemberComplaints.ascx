...
<script type="text/javascript">
    ...
    {
        var Member=$("#M_MemberComplaints_ctl00_TextBoxID").val();
        if(Member!="")
        {
            ...
            $.ajax({
                ...
                success: function (ajaxData) 
                {
                     if(ajaxData=="false")
                     {
                        ...
                        $("#M_MemberComplaints_ctl00_HiddenFieldMember").val("no");
                     }
                     else if(ajaxData=="true")
                     {
                        ...
                        $("#errMember").css("color","red");
                        $("#loading").css("display","none");
                        $("#M_MemberComplaints_ctl00_HiddenFieldMember").val("ok");
                     }
                }
                })
        }
    ...
    {
        var text=$("#M_MemberComplaints_ctl00_TextBoxOrderID").val();
        if(text!="")
        {
            $("#errUrl").html("*");
    ...
    {
        var type=$("#M_MemberComplaints_ctl00_DropDownListType").val();
        if(type=="-1")
        {
    ...
    {
        var Evidence=$("#M_MemberComplaints_ctl00_TextBoxEvidence").val();
        if(Evidence!="")
        {
            $("#errEvidence").html("*");
    ...
    {
        funCheckMember();
        if($("#M_MemberComplaints_ctl00_HiddenFieldMember").val()=="ok"&& funCheckOrderID()&&funCheckType()&&funCheckEvidence())
        {
            return true;
        }
        else
    ...
    $(function(){
            ...
            if(arry!=null)
            {
                //$("#M_MemberComplaints_ctl00_TextBoxID").val(arry.toString().split("|")[0]);
                $("#M_MemberComplaints_ctl00_TextBoxOrderID").val(arry.toString().split("|")[1]);
            }
    });
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="biaod">
    ...
    <tr>
        ...
        <td class="bordrig" style="border-bottom:1px solid #C6DFFF;">
            <asp:FileUpload ID="FileUploadImage" runat="server" name="fileField" /><span>上传凭证不超过1M，上传图片支持gif、jpg、png格式。</span>
        </td>
    </tr>
</table>
