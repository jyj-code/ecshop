...
<script type="text/javascript" language="javascript">
     function IsDelete(val){
         if(confirm("是否删除勾选项？"))
         {
               $.get("/Api/Main/Member/DeleteOp.ashx?type=SupplyDemandComment&delid="+val,null,function(data){
        ...
        <ItemTemplate>
            <tr>
                ...
                <td style=" text-align:left;">
                    <a target="_blank" href='<%# ShopUrlOperate.RetUrl("SupplyDemandDetail",Eval("SupplyDemandGuid")) %>'
                        target="_blank">
                        <%#Eval("Content").ToString().Length > 19 ? Eval("Content").ToString().Substring(0, 19) : Eval("Content").ToString()%></a>
                </td>
                <td>
                    <%#Eval("CreateTime")%>
                </td>
                ...
                <td class="th_ri">
                    <a href="M_SupplyDemandCommentDetail.aspx?guid=<%#Eval("Guid")%>">查看</a>
                </td>
            </tr>
        </ItemTemplate>
    </asp:Repeater>
