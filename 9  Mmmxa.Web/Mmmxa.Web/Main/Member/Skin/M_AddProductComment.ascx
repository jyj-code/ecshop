...
<style type="text/css">
  input,img,label{cursor:pointer;  }
</style>
<!--内容部分Content-->
  <div id="content_bg">
     <div class="con_order" >
         ...
         <div class="zjbk">
          <%if (dt != null)
            {%>
             ...
             <!--循环代码-->
               <dl class="zjpro" >
                   <dt><a href="#"><img src="<%=dt.Rows[i]["img"] %>" width="130" height="130"  /></a></dt>
                   <dd>
                   <a href="#" class="blue"><%=dt.Rows[i]["productname"] %></a>
                   </dd> 
               </dl>
                <div class="fabiaok1">
                   <div class="fbcommenttype"><span class="commenttype"><%=dt.Rows[i]["commenttype"]%></span><span class="commentcontent"><%=dt.Rows[i]["comment"]%></span><span class="commentdate">[<%=Convert.ToDateTime(dt.Rows[i]["commenttime"]).ToString("yyyy-MM-dd")%>]</span></div>
                   <div class="fbtext">
...
<script type="text/javascript" language="javascript">
    ...
    function subcheck()
    {
            ...
            $("textarea[name='TextBoxComment']").each(function(){
                ...
                {
                    $(this).focus(); alert("评论内容不能超过500字！");
                }
                pcontent.push($(this).val().replace(",",""));  xcheck.push($(this).attr("lang"));
            });
             $("#M_AddProductComment_ctl00_hidPGuid").val(xcheck.join(","));
           $("#M_AddProductComment_ctl00_hidCommentC").val(pcontent.join(","));
    }
</script> 