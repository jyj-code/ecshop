...
<head runat="server">
   ...
   return false;
        ...
        { var ThemeGuid=$("#ThemeGuid").val();
           ...
           if(checknum(pageindex))
           {
                ...
                if(pageEnd<=pageindex)
                    pageindex=pageEnd;
                location.href="S_ThemeProductList.aspx?ThemeGuid="+ThemeGuid+"&pageid="+pageindex;
           }
       }
    ...
    function GetData(pageid)
    {
        ...
        var textName=$("#S_ThemeProduct_ctl00_txtKeyWord").val();//商品名称
        ...
        $.ajax({
                ...
                async: false,
                data: "code="+code2+"&type=getproduct&keyword="+escape(textName)+"&pageid="+pageid,
                dataType: "html",
                success: function (ajaxData) 
                {
                    if(ajaxData!="")
                    {
                       ...
                       $.each(data,function(m,n){
                            ...
                            xhtml.push("<td>"+n.name.substring(0,20) +"</td>");
                           xhtml.push("<td>"+n.shopprice+"</td>");
                           xhtml.push("<td>"+n.repertorycount+"</td></tr>");
    ...
    function initMyPages(nowPage,pages){
           ...
           var endPage=pages;
            ...
            var pageNumStr="";
            if(nowPage!=1){
                pageNumStr+="<a href=\"javascript:;\" onclick='javascript:;GetData(1)'>首页</a><a href=\"javascript:;\" onclick=\"javascript:;GetAjaxContent("+(nowPage-1)+")\">上一页</a> ";
            }
            //判断什么时候显示下一页
            if(nowPage<pages){
                pageNumStr+=" <a href='javascript:;' onclick='javascript:;GetData("+(nowPage+1)+")'>下一页</a><a href='#' onclick='javascript:;GetAjaxContent("+pages+")'>末页</a>";
            }
    ...
    <div class="dpsc_mian">
        <p class="ptitle">
            <span class="spanrig" style="display:none"><a class="tianjiafl lmf_btn" href="S_ThemeProductOperate.aspx">
                添加商品</a> </span><a href="S_Welcome.aspx">我是卖家</a><span class="breadcrume_icon">>></span><a href="S_ThemeList.aspx">主题活动列表</a><span class="breadcrume_icon">>></span><span class="breadcrume_text">主题活动商品</span>
        </p>
        <ShopNum1ShopAdmin:S_ThemeProductList ID="S_ThemeProductList" PageSize="5" runat="server"
            SkinFilename="skin/S_ThemeProductList.ascx" />
    </div>
    </form>
</body>
</html>
