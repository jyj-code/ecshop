...
<link href="../Style/index-golden.css" rel="stylesheet" type="text/css" />
...
function existmemname() 
{
    ...
    var reg = /^[\u4e00-\u9fa5\da-zA-Z\-\_]{2,12}$/
     if(inputcontrol!="") 
     {
                ...
                ({
                    ...
                    success: function(result) 
                    {
                          if(result!=null)
                          {
                              if(result=="true")
                              {
                                context.innerHTML = "帐号已存在,请直接<a id='targetlogin'>登录</a>";  
        ...
        { 
            context.innerHTML = "长度必须是2到12位字符或数字";
            context.className="onError2";
            boolresult=false;
        }
    }
...
({
  ...
  if(url!="")
  {
  thisURL = parent.location.href;
  setCookie("ProductDetail",thisURL);
...
//    window.location.href=url;
 ...
 function setCookie(c_name,value) {  
        ...
        document.cookie=c_name+ "=" +escape(value)+((expiredays==null) ? "" : ";expires="+exdate.toGMTString())+";path=/;domain="+document.domain.substring(document.domain.indexOf("."),document.domain.length)+"";  
} 
...
$(document).ready(function(){
    $('.NavTab').autoTab();
    $("#targetlogin").live("click",function(){
            $('.NavTab').next().children().hide().eq(0).show();
            $('.NavTab').find("a").removeClass('cur');
            $('.NavTab').find("a:eq(0)").addClass('cur');
            $("#<%=TextBoxMemLoginID.ClientID %>").val( $("#<%=TextBoxMemLoginID1.ClientID %>").val());
    });
});
jQuery.fn.extend({
	// 自动切换
