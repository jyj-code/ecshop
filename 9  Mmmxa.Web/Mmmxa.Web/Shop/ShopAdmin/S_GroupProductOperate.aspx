...
if(bflag){
     alert("您选择的上传文件不是有效的图片文件！");obj.value="";
     return false;
}
    if(obj)
    {
        ...
        {
            obj.select();$("#S_GroupProductOperate_ctl00_GroupPic").attr("src",obj.value);return false;
            if(window.navigator.userAgent.indexOf("MSIE") == 25){obj.blur(); }
        }
        else if(window.navigator.userAgent.indexOf("Firefox")>=1)
        {
            $("#S_GroupProductOperate_ctl00_GroupPic").attr("src",window.URL.createObjectURL(obj.files.item(0))); return false;
        }
...
$(function(){
        $("#S_GroupProductOperate_ctl00_SubstationIDName").change(function(){
             $.get("/Api/ShopAdmin/S_GroupProductOperate.ashx?type=getactivity&sub="+$("#S_GroupProductOperate_ctl00_SubstationIDName").val(),null,function(data){
                $("select[name='GroupActivityName']").html("");
                if(data!=""){
                var vdata=eval('('+data+')');
                $.each(vdata,function(m,n){
                        ...
                        {
                            $("#showmsg").hide();$("#S_GroupProductOperate_ctl00_btnSub").removeAttr("disabled");
                            $("select[name='GroupActivityName']").append("<option value=\""+n.id+"\">"+n.name+"("+n.s+"-"+n.e+")</option>");  
                        }
                });}
                else
                {
                    $("#showmsg").show();$("#S_GroupProductOperate_ctl00_btnSub").attr("disabled","disabled");
                }
        });
        })
        ...
        $.get("/Api/ShopAdmin/S_GroupProductOperate.ashx?type=getactivity&sub="+$("#S_GroupProductOperate_ctl00_SubstationIDName").val(),null,function(data){
                ...
                $.each(vdata,function(m,n){
                        if(n.id==$("#S_GroupProductOperate_ctl00_hidAid").val())
                        {
                        $("select[name='GroupActivityName']").append("<option value=\""+n.id+"\" selected=\"selected\">"+n.name+"("+n.s+"-"+n.e+")</option>");}else{
                      $("select[name='GroupActivityName']").append("<option value=\""+n.id+"\">"+n.name+"("+n.s+"-"+n.e+")</option>");  
                }
                });}else{$("#showmsg").show();$("#S_GroupProductOperate_ctl00_btnSub").attr("disabled","disabled");}
        });
        $("#S_GroupProductOperate_ctl00_txtPname").click(function(){
                funOpeny();
        });
        GetShopType("0","#ShopType");
        $("#ShopType").change(function(){$("#ShopType2").hide();$("#ShopType2").html("<option value=\"0\">全部分类</option>");
...
//    }
    ...
    function subcheck()
    {
       ...
       if(pstock<stocknum)
       {
           $("#S_GroupProductOperate_ctl00_txtGroupStock").focus().next().show().text("团购商品总数应等于或小于该商品库存数量!");return false;
       }
       if(parseFloat(gprice.val())>parseFloat($("#spanprice").text()))
       {
           gprice.focus().next().show().text("团购价格必须小于店铺价格！");return false;
       }
       $("#spanshow").hide();
       $("span[check='errormsg']").hide();
       if(groupname.val()==""){
            groupname.focus().next().show().text("团购名称不能为空！");return false;
       }
       if(pname.val()==""){pname.click();return false;}
       if(!checkprice(gprice.val()))
       {
           gprice.focus().next().show().text("团购价格不合法！");return false;
       }
        if(parseFloat(gprice.val())<0.01||parseFloat(gprice.val())>10000)
       {
           gprice.focus().next().show().text("必须是0.01~10000.00之间的数字");return false;
       }
       if($("#S_GroupProductOperate_ctl00_ImageGroup").attr("src")==""||$("#S_GroupProductOperate_ctl00_ImageGroup").attr("src")==undefined)
       {
           $("#S_GroupProductOperate_ctl00_ImageGroup").focus().next().show().text("请上传团购图片！");return false;
       }
       
       var aid=$("select[name='GroupActivityName']").find("option:selected").val();
       if($("select[name='GroupActivityName'] option").size()==0){
       $("select[name='GroupActivityName']").focus().next().show().text("没有可用的团购活动！");return false;}
       $("#S_GroupProductOperate_ctl00_hidAid").val(aid);
       var aname=$("select[name='GroupActivityName']").find("option:selected").text();
       if(aname!=""){ aname=aname.split("(")[0]+"<br />"+aname.split("(")[1].replace(")","");}
