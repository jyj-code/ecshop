     ...
     function IsDelete(val){
          if(confirm("是否删除勾选项？"))
          {
            $.get("/Api/ShopAdmin/S_DeleteOp.ashx?type=ScoreProduct&delid="+val,null,function(data){
                    if(data=="ok"){alert("删除成功！");location.reload();window.location.href="S_ScoreProduct.aspx"; return false;}
        ...
        <ItemTemplate>
            <tr>
                ...
                <td style="line-height: 18px;">
                    <a target="_blank" href='<%#ShopUrlOperate.shopDetailHrefNew(Eval("Guid").ToString(),Eval("MemLoginID").ToString(),"ProductIntegral")%>'
                        class="sroceimg">
