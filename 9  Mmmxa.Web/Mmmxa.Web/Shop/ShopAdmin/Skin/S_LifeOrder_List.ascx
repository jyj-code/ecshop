...
<% DataTable dt = S_LifeOrder_List.dt_OrderList; DataTable dt_sub = null;  %>
...
<div id="list_main">
    ...
    <div id="content" class="ordmain">
        ...
        <%if (dt != null)
          {
              dt_sub = S_LifeOrder_List.dt_GetOrderProduct(""); bool bflag = true;
        ...
        <table width="100%" border="0" cellspacing="1" cellpadding="5" class="blue_tb1" id="ord_<%=i%>"
            ...
            <tr>
                ...
                <td width="200" style="text-align: left;">
                    <a href="<%=strProductUrl %>" class="alink_blue" target="_blank" title="<%=strProductName %>">
                        <%=ShopNum1.Common.Utils.GetUnicodeSubString(strProductName,20,"...")%></a><br />
                    <span>
...
<script type="text/javascript" language="javascript">
      ...
      function surecheck(guid,ordernumber,payname,memloginid)
        {
            window.parent.scrollTo(0,0);
             funOpen();
             $(".showpoptwo").show();
             $("#S_LifeOrder_List_ctl00_hidOrderGuId").val(guid);
        ...
        $(function(){
           $("#sureupdate").click(function(){
                if($("#txtNewPrice").val()==""){$("#tipmsg").show().text("修改的订单价格不能为空！");return false;}
                else if(!checkpriceTxt($("#txtNewPrice"))){$("#tipmsg").show().text("修改的订单价格不合法！");return false;}
                else{
    ...
    function checkpriceTxt(o)
    {
        ...
        if(!oo.test(o.val())){$("#tipmsg").show().text("修改的订单价格不合法！");return false;}else{ o.val(number_format(o.val(),2));return true;}
    }
</script>
