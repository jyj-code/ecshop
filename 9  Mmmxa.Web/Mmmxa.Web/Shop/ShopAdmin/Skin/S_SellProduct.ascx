...
//           });
           ...
           //Ajax动态加载商品类型
           var ctype='<%=Common.ReqStr("stype") %>';
           $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=1",null,function(data){
                     var vdata=eval('('+data+')');
                     var ts=new Array();
                     $.each(vdata,function(m,n){
                           if(ctype==n.id){ts.push("<option value=\""+n.id+"\" selected=\"selected\">"+n.name+"</option>");}else{
                           ts.push("<option value=\""+n.id+"\">"+n.name+"</option>");}
                     });
                     $("select[name='selCateType']").append(ts.join(""));
           });
           //全选反选
           ...
           $(".shanchu").click(function(){
                    ...
                    var arryid=new Array();var bflag=false;
                     $("input[name='subcheck']").each(function(){
                            if($(this).is(":checked")){arryid.push($(this).val());bflag=true;}
                     });
                     if(!bflag)
                     {
                            alert("请选择一件商品进行删除操作！");return false;
                     }
                     if(confirm("是否执行批量删除操作？")){
                      $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=3&ids="+arryid.join(","),null,function(data){
                             for(var i in arryid)
                             {
                                $("#tab_"+arryid[i]).hide();$("#Maintab_"+arryid[i]).hide();
                             }
                      });
                     alert("删除成功！");$("input[name='subcheck']").removeAttr("checked");window.location.href="S_SellProduct.aspx";
                     $("#tipmsg").show().find("font").text("删除成功！");
                     }
           });
           //下架操作方法
           $(".xiaj").click(function(){
                    var arryid=new Array();var bflag=true;
                     $("input[name='subcheck']").each(function(){
                           if($(this).is(":checked")){arryid.push($(this).val());bflag=false;}
                     });
                     if(bflag)
                     {
                        alert("请勾选一条数据进行下架操作！");return false;
                     }
                     
                     if(confirm("是否执行下架操作？")){
                        $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=4&sale=0&ids="+arryid.join(","),null,function(data){});
                        alert("下架成功！");$("input[name='subcheck']").removeAttr("checked");window.location.href="S_SellProduct.aspx";
                       $("#tipmsg").show().find("font").text("下架成功！");
                       for(var i in arryid)
                       {
                         $("#tab_"+arryid[i]).hide(); $("#Maintab_"+arryid[i]).hide();
                       }
           ...
           $(".nosbe").dblclick(function(){
                $(this).find(".edit_name").hide();$(this).find(".edit_v").show();
           });
           //保存修改
           $(".btn_save").click(function(){
                if($(this).prev().val().length>50||$(this).prev().val().length<3&&$(this).attr("name")=="name")
                {
                    alert("商品标题名称长度至少3个字符，最长50个汉字!");return false;
                }
    ...
    function GetShopProductCategory(id,divid)
    {
         $.get("/Api/ShopAdmin/S_SellProduct.ashx?type=5&parentid="+id,null,function(data){
               var vdata=eval('('+data+')');
                  var ts=new Array();
                  $.each(vdata,function(m,n){ 
                      var ccode='<%=Common.ReqStr("shopct") %>';var cvcode='<%=Common.ReqStr("shopct") %>';
                      if(ccode!="")
                      ...
                      else if(ccode==n.code){
                                ts.push("<option value=\""+n.code+"\" lang=\""+n.id+"\" selected=\"selected\">"+n.name+"</option>");
                                GetShopProductCategory(n.id,"#selectshopType2");
                      }
                      else if(cvcode!=n.code){ts.push("<option value=\""+n.code+"\" lang=\""+n.id+"\">"+n.name+"</option>");}
                    });
    ...
    function ontoPage(o)
    {
       ...
       if(checknum(pageindex))
       {
             ...
             if(pageEnd<=pageindex)
                    pageindex=pageEnd;
            location.href="S_SellProduct.aspx?pageid="+pageindex;
       }
   }
  ...
  function checkpriceTxt(o)
    {
        ...
        if(!oo.test(o.val())){return false;}else{
...
<div id="list_main">
    <div class="ordmain1" id="content">
            ...
            <table border="0" cellspacing="0" cellpadding="0" class="lineh">
                <tbody>
                    <tr class="up_spac">
                        ...
                        <td>
                            <input type="text" class="ss_nr1" name="ProductName" value="<%=Common.ReqStr("pn").Replace("%2f","/") %>" />
                        </td>
                    ...
                    <tr class="up_spac">
                        ...
                        <td colspan="3">
                            <table cellpadding="0" cellspacing="0" border="0">
                                <tr>
                                    <td>
                                        <input type="text" class="ss_nr1" name="Number" value="<%=Common.ReqStr("no").Replace("%2f","/") %>" />
                                    </td>
                                    <td>
        ...
        <div class="btntable_tbg">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="btntable_top">
                <tr>
                    ...
                    <td width="6%" style="border-right: none; text-align: left; padding-left: 10px;">
                        <label for="topCheck">
                            全选</label>
                    </td>
                    <td width="90%">
                        ...
                        <div class="shanc1">
                            <a class="xiaj lmf_btn" href="#">下架</a>
                        </div>
                        <div class="shanc1 hideme" id="tipmsg">
        ...
        <%DataTable dt = S_SellProduct.dt_ShowProduct;
          if (dt != null)
          {
              for (int i = 0; i < dt.Rows.Count; i++)
              {
                  DataRow dr = dt.Rows[i];
                  string strProductUrl = ShopUrlOperate.shopHrefNew(dr["guid"].ToString(), dr["memloginId"].ToString());
                  if(dr["Productstate"].ToString()=="1")
                      strProductUrl = ShopUrlOperate.shopDetailHrefByShopIDNew(dr["guid"].ToString(), dr["ShopId"].ToString(), "ProductIsPanic_Detail");
        %>
        <!--循环标签代码-->
        <table width="100%" cellspacing="0" id="Maintab_<%=dr["id"]%>" cellpadding="5" border="0"
            class="blue_tb12" style="margin-bottom: 0;">
            <tr>
                <th style="border-left: solid 1px #e3e3e3; width: 48px; text-align: right;">
                    <div class="ord_check" style="padding-top: 0;">
                        <input type="checkbox" value="<%=dr["id"]%>" id="fx" name="subcheck"></div>
                </th>
                <th style="text-align:left; border-right: solid 1px #e3e3e3;">
                    货号：<span class="blue"><%=dr["productnum"]%></span>
                    店铺商品分类：<%=ShopNum1.Common.Utils.GetUnicodeSubString(dr["productseriesname"].ToString(),45,"...")%>&nbsp;&nbsp;
                    添加时间：<%=dr["ModifyTime"]%>&nbsp;&nbsp;已售：<%=dr["salenumber"]%>件
                </th>
            </tr>
        </table>
        <table width="100%" cellspacing="0" id="tab_<%=dr["id"]%>" cellpadding="5" border="0"
            class="blue_tb12" style="margin-top: 0;">
            <tbody>
                <tr>
                    <td style="border-left: solid 1px #e3e3e3;" colspan="2">
                        <a href="<%=strProductUrl %>"
                            class="alink_blue" target="_blank">
                            <img width="65" height="66" src="<%=dr["originalimage"]%>_60x60.jpg" width="60" height="60"
                    ...
                    <td style="width: 180px; padding: 0px;">
                        <div class="nosbe">
                            <span class="edit_name" title="双击编辑">
                                <%=ShopNum1.Common.Utils.GetUnicodeSubString(dr["name"].ToString(), 15, "")%></span><span
                                    class="edit_v" style="display: none">
                        ...
                        <div class="nosbe">
                            <span class="edit_name" title="双击编辑">
                                <%=dr["shopprice"]%></span><span class="edit_v" style="display: none">
                                    <input type="text" value="<%=dr["shopprice"]%>" class="edit_txt" maxlength="6" onblur="checkpriceTxt(this)" />
                    ...
                    <td style="width: 110px;">
                        <a class="alink_blue" href="javascript:void(0)" onclick="if(confirm('是否删除？')){location.href='?del=<%=dr["id"]%>&sign=del'}">
                            删除</a> <a class="alink_blue" href="<%=strProductUrl %>" target="_blank">查看</a>
