    ...
    $(function(){
     ...
     if(confirm("是否删除勾选项？"))
     {
        $.get("/Api/ShopAdmin/S_DeleteOp.ashx?type=ZtcGoods&delid="+val,null,function(data){
                    if(data=="ok"){alert("删除成功！");location.reload();window.location.href="S_ZtcGoods_List.aspx"; return false;}
            ...
            <asp:Repeater ID="RepeaterShow" runat="server">
                <ItemTemplate>
                    <tr>
                        ...
                        <td>
                            <a target="_blank" href='<%#ShopUrlOperate.shopHrefNew(((DataRowView)Container.DataItem).Row["ProductGuid"].ToString(),((DataRowView)Container.DataItem).Row["MemberID"].ToString() )%>'
                        ...
                        <td width="60">
                            <%#ShopNum1.ShopAdminWebControl.S_ZtcGoods_List.Expire(Eval("Ztc_Money").ToString()) == "余额不足" ? "<font color=red>余额不足</font>" : ShopNum1.ShopAdminWebControl.S_ZtcGoods_List.Expire(Eval("Ztc_Money").ToString())%>
                        </td>
                        <td width="30">
