    ...
    $(function(){
     ...
     if(confirm("是否删除勾选项？"))
     {
            $.get("/Api/ShopAdmin/S_DeleteOp.ashx?type=ShopCoupon&delid="+val,null,function(data){
                    if(data=="ok"){alert("删除成功！");location.reload();window.location.href="S_ShopCoupon.aspx"; return false;}
            ...
            <asp:Repeater ID="RepeaterShow" runat="server">
                <ItemTemplate>
                    <tr>
                        ...
                        <td>
                            <a target="_blank" href='<%# ShopUrlOperate.shopDetailHrefNew(Eval("Guid").ToString(),Eval("MemLoginID").ToString(),"Coupon") %>' title="<%#Eval("CouponName") %>">
                                <%#Utils.GetUnicodeSubString(Eval("CouponName").ToString(), 10, "")%>
                            
                        </td>
                        <td>
                            <a title="<%#ShopNum1.ShopAdminWebControl.S_ShopCoupon.GetNewsCategoryName(Eval("Type").ToString()) %>">
