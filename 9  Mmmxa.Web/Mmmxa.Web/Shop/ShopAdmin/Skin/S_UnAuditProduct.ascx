           ...
           $(".shanchu").click(function(){
                if(confirm("是否执行批量删除操作？")){
                    var arryid=new Array();var bflag=false;
                     ...
                     if(!bflag)
                     {
                        alert("请选择一件商品进行操作！");return false;
                     }
                     ...
                     $("#tipmsg").show().find("font").text("删除成功！");     $("input[name='subcheck']").removeAttr("checked");window.location.href="S_UnAuditProduct.aspx";
    ...
    function ontoPage(o)
    {
       ...
       if(checknum(pageindex))
       {
             ...
             if(pageEnd<=pageindex)
                    pageindex=pageEnd;
            location.href="S_UnAuditProduct.aspx?pageid="+pageindex;
       }
   }
...
<div id="list_main">
    <div class="ordmain1" id="content">
            ...
            <table width="100%" cellspacing="0" cellpadding="0" border="0" class="btntable_top">
                <tr>
                    ...
                    <td width="90%">
                        ...
                        <div class="shanc1">
                            <a class="shangj lmf_btn" href="#" style="display: none;">上架</a>
                        </div>
        ...
        <%
            DataTable dt = S_UnAuditProduct.dt_ShowProduct;
            if (dt != null)
            {
        ...
        %>
        <!--循环标签代码-->
        <table width="100%" cellspacing="0" cellpadding="0" border="0" class="blue_tb12"
            style="border-bottom: none; margin-bottom: 0;" id="Maintab_<%=dr["id"]%>">
            <tr>
                <th style="border-left: solid 1px #e3e3e3; width: 26px; text-align: right;">
                ...
                <th style="text-align: left; border-right: solid 1px #e3e3e3;">
                    货号：<span class="blue"><%=dr["productnum"]%></span> 店铺商品分类：<%=ShopNum1.Common.Utils.GetUnicodeSubString(dr["productseriesname"].ToString(),45,"...")%>&nbsp;&nbsp;
                    添加时间：<%=dr["ModifyTime"]%> &nbsp;&nbsp; &nbsp;&nbsp;<%=dr["IsAudit"].ToString()=="2"?"<font color='red'>审核未通过</font>":"暂未审核"%>
                </th>
            </tr>
        </table>
        <table width="100%" id="tab_<%=dr["id"]%>" cellspacing="0" cellpadding="0" border="0"
            class="blue_tb12" style="border-top: none; margin-top: 0;">
            <tr>
                ...
                <td style="width: 110px;">
                    <a class="alink_blue" href="javascript:void(0)" onclick="if(confirm('是否删除？')){location.href='?del=<%=dr["id"]%>&sign=del'}">
                        删除</a> <a class="alink_blue" target="_blank" href="S_SellGoods_two.aspx?op=edit&ctype=<%=dr["ctype"] %>&pid=<%=dr["guid"] %>&code=<%=dr["productcategorycode"]%>">
                            编辑</a>
                </td>
            </tr>
        </table>
