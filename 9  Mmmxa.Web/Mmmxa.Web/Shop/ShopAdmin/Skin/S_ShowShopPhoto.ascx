...
<script type="text/javascript" language="javascript">
    ...
    {
        var txtid='<%=Common.ReqStr("txtid")%>';
        var imgid='<%=Common.ReqStr("imgid")%>';
       try{
        window.parent.document.getElementById(imgid).src=imageurl;
        window.parent.document.getElementById(txtid).value=imageurl; 
        try{window.parent.document.getElementById("showImg").style.display="";}catch(e){} 
        ...
        window.parent.document.getElementById("sp_overlayDiv2").style.display="none";
           }catch(e){ window.parent.document.getElementById("sp_dialog_contDiv").style.display="none";
                   ...
                   window.parent.document.getElementById("showpop1").style.display="none";
                   window.parent.document.getElementById("showpop2").style.display="none";
           }
                try{window.parent.openSingleChild(imageurl);}catch(e){}
    }
	...
	x : 10,	y : 20,tipElements : ".editname",	init : function() {
     ...
     function subSearch()
     {
        ...
        var imgname=$("#S_ShowShopPhoto_ctl00_txtImageName").val();
         var txtid='<%=Common.ReqStr("txtid")%>';
        var imgid='<%=Common.ReqStr("imgid")%>';
        location.href="/Shop/ShopAdmin/S_ShowShopPhoto.aspx?txtid="+txtid+"&imgid="+imgid+"&st="+selecttype+"&imgname="+escape(imgname)+"";return false;
     }
    </script>
<table cellpadding="0" cellspacing="0" border="0" style="padding-top: 10px; z-index:100px;">    
    <tr class="up_spac">
        ...
        <td>
            <asp:Button ID="ButtonChaxData" runat="server" OnClientClick="return subSearch()" Text="查询" CssClass="chax btn_spc"
    ...
    <tr class="up_spac">   
        ...
        <td colspan="2" style="color:Silver; text-align:center;">
            (上传图片要小于1M!)
        </td>
    </tr>
</table>
                ...
                <li>
                    <div class="border_img">
                        <a onclick='funReturn("<%#Eval("ImagePath")%>")'>
                            <img src='<%#Eval("ImagePath").ToString().StartsWith("~/") == true ? Eval("ImagePath").ToString().Substring(1) : Eval("ImagePath").ToString()%>' /></a></div>
                    <div class="img_title">
                        <a href="#"><%#Eval("Name").ToString().Length > 6 ? Eval("Name").ToString().Substring(0, 6) : Eval("Name").ToString()%></a></div>
                </li>
            </ItemTemplate>
        </asp:Repeater>
