    ...
    <div class="box_content">
        ...
        <div class="control_group">
            <label for="picurl" class="control_label">
                图文封面：</label>
            <div class="controls">
                <asp:Image ID="thumb_img" Style="max-height: 100px; display: none;" runat="server"
                    ImageUrl="/ImgUpload/noimg.jpg_160x160.jpg" />
                <asp:HiddenField ID="thumb" Value="" runat="server" />
                <span class="help_inline"><a class="btn" id="insertimage">选择图文封面</a></span>  <span class="maroon1">*</span>建议大小(宽720高400)
            </div>
        </div>
        <div class="control_group">
        ...
        <div class="control_group">
            ...
            <div class="controls">
                ...
                <asp:TextBox ID="FCKeditorRemark" runat="server" TextMode="MultiLine" name="reply_content"
                    class="Texteditor" Style="width: 586px; height: 300px; visibility: hidden;"></asp:TextBox>
                <asp:TextBox ID="txtLink" runat="server" name="replyLink_content" CssClass="textwb"  Style="width: 386px; height: 20px;"></asp:TextBox>(为了保证链接准确无误,请以http开头!)
            </div>
        </div>
        <div class="control_group">
            <label for="picurl" class="control_label">
...
<script type="text/javascript">
    ...
    KindEditor.ready(function(K) {
        ...
        K('#insertimage').click(function() {hideSet();
        ...
        $("input[name='W_ShopReplyImgTxt$ctl00$article']").each(function(){
                if($(this).is(":checked")&&$(this).val()=="0")
                {
                    $(".ke-container-default").hide();$("#W_ShopReplyImgTxt_ctl00_txtLink").show();
                }else
                {
                  $(".ke-container-default").show();$("#W_ShopReplyImgTxt_ctl00_txtLink").hide();
                }
        });
        
        $("input[name='W_ShopReplyImgTxt$ctl00$article']").click(function(){
                if($(this).val()=="0")
                {
                   $(".ke-container-default").hide();$("#W_ShopReplyImgTxt_ctl00_txtLink").show();
                }else
        ...
        $("#btn_Save").click(function() {
            ...
            //文章类型
            var articltype = $(".alert input[type=radio]:checked").val()
            //文章内容
            editor.sync();
            var articlecontent = $("#<%=FCKeditorRemark.ClientID %>").val();

            var urllink= $("#<%=txtLink.ClientID %>").val();
            if(msgtype=="0")
                 articlecontent=urllink;
            //多图文
            var chirldids = "";
...
//            alert($("input[name='W_ShopReplyImgTxt$ctl00$article']:checked").val()+"|"+articlecontent);
