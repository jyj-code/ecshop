...
<div style="border: 1px #e0e0e0 solid; background: #f3f3f3; height: 30px; line-height: 30px;
    padding: 0 15px;">
    <table border="0" cellspacing="0" cellpadding="0" width="100%" class="lineh" style="margin-top: 0;
        padding-left: 0;">
        <tr>
            <td style="font-weight: bold; color: #ff6600; padding-right: 10px; font-size: 14px;
                height: 30px; line-height: 30px; vertical-align: top; width: 300px; display:block; overflow:hidden;">
            ...
            <td style="height: 30px; line-height: 30px; vertical-align: top; padding-left: 6px; white-space:nowrap;">
                <label for="selectall">
                    全选</label>
            </td>
            <td style="height: 30px; line-height: 30px; vertical-align: top; padding-left: 10px;
                padding-top: 4px;">
            ...
            <td width="160" style="height: 30px; line-height: 30px; vertical-align: top; padding-top: 6px;*padding-top:0px;">
                <select id="imgtype_sort" name="sort">
                    <option value="">-全部-</option>
                    <option value="0" <%=Common.ReqStr("sort")=="0"?"selected='selected'":"" %>>按上传时间从晚到早</option>
                    <option value="1" <%=Common.ReqStr("sort")=="1"?"selected='selected'":"" %>>按上传时间从早到晚</option>
                    <option value="2" <%=Common.ReqStr("sort")=="2"?"selected='selected'":"" %>>按图片从大到小</option>
                    <option value="3" <%=Common.ReqStr("sort")=="3"?"selected='selected'":"" %>>按图片从小到大</option>
                    <option value="4" <%=Common.ReqStr("sort")=="4"?"selected='selected'":"" %>>按图片名降序</option>
                    <option value="5" <%=Common.ReqStr("sort")=="5"?"selected='selected'":"" %>>按图片名升序</option>
                </select>
            </td>
            <td width="90" align="right">
                ...
                <script type="text/javascript" language="javascript">
                                     $("#fileupload").uploadify({
                                        ...
                                        'script': '/Api/uploadify.ashx?albumid=<%=Encryption.Encrypt(hidShopId.Value) %>-<%=Common.ReqStr("typeid")%>&shopid=dd',
                                        ...
                                        {   
                                            alert("您成功上传了"+data.filesUploaded+"张图片！");window.location.reload(true);
                                        }
                                 });
                ...
                <div id="showupfiles" style="z-index: 99999; display: none; position: absolute; border: 1px solid #bfbfbf;
                    width: 360px; top: 150px; left: 430px; background: #fff;">
...
<ul class="picture-list">
    ...
    <li class="normal">
        <dl style="height: auto;">
            <dt style="padding: 0; background: none; height:auto;">
                <div class="picture" style="width: 166px; height: 166px; border: none;">
                ...
                <table cellpadding="0" cellspacing="0" border="0" style="padding-top: 3px; margin: 0 auto;">
                    <tr>
                        <td>
                        ...
                        <td style="padding-left: 8px; width:150px;" class="showtxt">
                            <label style="display:none;" id="showlbl_<%=dt_list.Rows[i]["id"] %>"><%=dt_list.Rows[i]["name"] %></label>
                            <input type="text" id="showtxt_<%=dt_list.Rows[i]["id"] %>" value='<%=dt_list.Rows[i]["name"] %>' class="editInput1" disabled="disabled" maxlength="12" />
                        </td>
                    </tr>
                </table>
            </dt>
            ...
            <dd class="buttons" style="text-align:center;">
                ...
                <a class="rename" href="JavaScript:void(0);" lang="<%=dt_list.Rows[i]["id"] %>">修改名称</a>
                 <%if (Common.ReqStr("typeid") != "0")
                  { %>
                <a class="cover" onclick="coverpic('<%=dt_list.Rows[i]["id"]%>','<%=Common.ReqStr("typeid")%>','<%=dt_list.Rows[i]["imagepath"].ToString().Replace("/","-")%>')"
                    href="JavaScript:void(0);">设为封面</a> <%} %><a onclick="if(confirm('您确定进行该操作吗?\n注意：使用中的图片也将被删除')){location.href='?act=del&typeid=<%=Common.ReqStr("typeid") %>&id=<%=dt_list.Rows[i]["id"]%>&Imgpath=<%=dt_list.Rows[i]["imagepath"].ToString().Replace("/","-") %>&sign=del';}"
...
<script type="text/javascript" language="javascript">
         ...
         $(function(){
                ...
                var sweetTitles = {
	                x : 10,	y : 20,tipElements : ".picture",	init : function() {
		                $(this.tipElements).mouseover(function(e){
			                var myTitle=$(this).parent().next().html();  var tooltip = "";
                ...
                $("#imgtype_sort").change(function(){
                      ...
                      {location.href='?typeid=<%=Common.ReqStr("typeid")%>&pageid=1';}else{ location.href='?sort='+$(this).find("option:selected").val()+'&typeid=<%=Common.ReqStr("typeid")%>&pageid=1';}
                 });
                $("#open_uploader a").toggle(function(){$("#showupfiles").show();$(this).text("取消上传");},function(){$("#showupfiles").hide();$(this).text("上传图片");});
                $("#setalbum strong").toggle(function(){$(this).text("取消批量处理"); $("#divbatch").show();},
                function(){$(this).text("批量处理");$("#divbatch").hide();});
