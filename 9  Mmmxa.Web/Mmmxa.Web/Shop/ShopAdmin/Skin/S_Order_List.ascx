...
<% DataTable dt = S_Order_List.dt_OrderList; DataTable dt_sub = null;  %>

<script type="text/javascript" language="javascript">
       $(function(){
                //搜索
               $("#sk").click(function(){
                        var state='<%=Common.ReqStr("st") %>';
                        location.href="S_Order_List.aspx?pn="+escape($("#pname").val())+"&ord="+$("#ordernum").val()+"&sd="+$("#txtstartdate").val()+"&ed="+$("#txtenddate").val()+"&mid="+escape($("#memloginid").val())+"&st="+state+"&";
               });
                 // 全选
...
<script type="text/javascript" language="javascript">
        ...
        {  window.parent.scrollTo(0,0);
           funOpen();
           $(".showpopone").show();$("#sp_dialog_outDiv h4").text("修改费用");
        ...
        $(function(){
           $("#sureupdate").click(function(){
                if($("#txtNewPrice").val()==""){$("#tipmsg").show().text("修改的订单金额不能为空！");return false;}
                else if(!checkpriceTxt($("#txtNewPrice"))){$("#tipmsg").show().text("修改的订单金额不合法！");return false;}
                else{
                       $($("#hidpriceid").val()).text($("#txtNewPrice").val());$("#tipmsg").show().attr("style","color:green;").text("修改成功！");$("#sureupdate").attr("disabled","disabled");
    ...
    function checkpriceTxt(o)
    {
        ...
        if(!oo.test(o.val())){$("#tipmsg").show().text("修改的订单金额不合法！");return false;}else{ o.val(number_format(o.val(),2));return true;}
    }
</script>

<div id="list_main">
    <ul class="sidebar">
        <li class='<%=Common.ReqStr("stype")=="0"||Common.ReqStr("stype")==""?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?stype=0">所有订单</a></li>
        <li class='<%=Common.ReqStr("stype")=="1"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=0&sstate=0&pstate=0&stype=1">等待付款</a></li>
        <li class='<%=Common.ReqStr("stype")=="2"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=1&sstate=0&pstate=1&stype=2">已付款</a></li>
        <li class='<%=Common.ReqStr("stype")=="3"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=2&sstate=1&pstate=1&stype=3">已发货</a></li>
        <li class='<%=Common.ReqStr("stype")=="6"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=3&sstate=2&pstate=1&stype=6">交易成功</a></li>
        <li class='<%=Common.ReqStr("stype")=="5"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=1&sstate=0&pstate=3&stype=5">已退款</a></li>
        <li class='<%=Common.ReqStr("stype")=="8"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
            <a href="S_Order_List.aspx?ostate=2&sstate=4&pstate=0&stype=8">已退货</a></li>
        <li class='<%=Common.ReqStr("stype")=="7"?"TabbedPanelsTab TabbedPanelsTabSelected":"TabbedPanelsTab" %>'>
    ...
    <div id="content" class="ordmain">
        ...
        <div class="pad">
            <table border="0" cellspacing="0" cellpadding="0" class="lineh">
                <tr class="up_spac">
                    ...
                    <td>
                        <input name="ordernum" id="ordernum" type="text" class="ss_nr1" value='<%=Common.ReqStr("ord") %>' />
                    </td>
                    <td align="right" class="ss_nr_spac">
                        商品名称：
                    </td>
                    <td>
                        <input name="pname" type="text" id="pname" class="ss_nr1" value='<%=Common.ReqStr("pn") %>' />
                    </td>
                </tr>
                <tr class="up_spac">
                    <td align="right">
                    ...
                    <td>
                        <table cellpadding="0" cellspacing="0" border="0">
                            <tr>
                                <td>
                                    <input value='<%=Common.ReqStr("sd") %>' name="startdate" type="text" id="txtstartdate"
                                ...
                                <td>
                                    <input value='<%=Common.ReqStr("ed") %>' name="enddate" type="text" id="txtenddate"
                                        class="ss_nrduan" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'})" />
                                </td>
                            </tr>
                        </table>
                    ...
                    <td>
                        <input name="memloginid" id="memloginid" type="text" class="ss_nr1" value='<%=Common.ReqStr("mid") %>' />
                    </td>
                    <td>
                 ...
                 <tr class="up_spac">
                    <td colspan="5"> 
                       <asp:Label runat="server" ID="lbTotalOrder"></asp:Label>&nbsp;&nbsp;&nbsp;
                       <asp:Label runat="server" ID="lbTotalMoney"></asp:Label>
                    </td>
        ...
        <div class="btntable_tbg">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="btntable_top">
                <tr>
                    ...
                    <td width="6%" style="text-align: left; padding-left: 10px;display:none;">
                        <label for="checktop1">
                            全选</label>
                    </td>
                    <td width="90%">
        ...
        <%if (dt != null)
          {
              dt_sub = S_Order_List.dt_GetOrderProduct(""); bool bflag = true;
              for (int i = 0; i < dt.Rows.Count; i++)
              {
                  ...
                  string strPayMentState = dr["paymentstatus"].ToString();
                  string strRefundState = dr["refundstatus"].ToString();
            ...
            <tr>
                <th colspan="5" class="th_le">
                    <div class="ord_check" style="display:none;">
                        <input name="checksub" type="checkbox" value="<%=dr["guid"] %>" lang="<%=i%>" vname='<%=dr["shopid"] %>' />
                    </div>
                    &nbsp;订单编号：<a class="alink_blue"><%=strOrderNumber%></a>&nbsp;&nbsp;&nbsp; 买家：<%=strMemloginId%>&nbsp;&nbsp;&nbsp;<strong>下单时间：<%=strCreateTime%></strong>
                </th>
                <th class="th_color th_ri">
                    订单状态：<span><%=S_Order_List.setOrderState(strOrderState, strPayMentState, strRefundState)%></span>
                </th>
            </tr>
        </table>
        <!--列表内容-->
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="blue_tb1" id="ord_<%=i%>"
            ...
            <%
                for (int j = 0; j < drr.Length; j++)
                {
                    ...
                    string strspecificationName = drr[j]["specificationName"].ToString().ToString().Replace("%2f", "/");
                    string strBuyPrice = drr[j]["ShopPrice"].ToString();
                    string strBuyNumber = drr[j]["BuyNumber"].ToString();
                    string strProductUrl = ShopUrlOperate.shopHrefNew(drr[j]["productguid"].ToString(), drr[j]["shopid"].ToString());
            %>
            <tr>
                <td width="82">
                ...
                <td width="200" style="text-align: left;">
                    <a href="<%=strProductUrl %>" class="alink_blue" target="_blank"  title="<%=strProductName %>">
                        <%=ShopNum1.Common.Utils.GetUnicodeSubString(strProductName,25,"...")%></a><br />
                    <span>
                        <%=strspecificationName.ToString().Replace("%2F", "/").Replace("0", "")%></span>
                </td>
                <td style="width: 70px;">
                    <span class="redbold">
                        <%=strBuyPrice%></span>
                ...
                <%if (j == 0)
                  { %>
                <td rowspan="<%=drr.Length %>" width="80">
                    <%if (dr["vrefund"].ToString() == "0" && dr["refundtype"].ToString() == "0" && strOrderState == "1")
                      { %>
                    <a href="S_index.aspx?tosurl=S_RefundGoods.aspx?view=exitmoney|<%=drr[j]["shopid"] %>|<%=dr["guid"] %>|commonorder&"
                        target="_blank" class="redv">买家申请退款</a>
                    <%}
                      else if (dr["vrefund"].ToString() == "0" && dr["refundtype"].ToString() == "1" && strOrderState == "2")
                      { %>
                    <a href="S_index.aspx?tosurl=S_RefundGoods.aspx?view=exitgoods|<%=drr[j]["shopid"] %>|<%=dr["guid"] %>|commonorder&"
                ...
                <td rowspan="<%=drr.Length %>" style="width: 70px;">
                    ...
                    <%if (dr["ordertype"].ToString() == "1")
                      { %>
                    <div style="color: Red">
                        <img src="images/tg.jpg" alt="团购活动" /></div>
                    <%}
                      else if (dr["ordertype"].ToString() == "2")
                      { %><div style="color: Red">
                          <img src="images/xs.jpg" alt="限时折扣" /></div>
                    <%}
                      else if (dr["ordertype"].ToString() == "3")
                      {%><div style="color: Red">
                          <img src="images/zh.jpg" alt="组合套餐" /></div>
                    <% }
                      else if (dr["ordertype"].ToString() == "4")
                      {%><div style="color: Red">
                          <img src="images/qg.jpg" alt="抢购活动" /></div>
                    <% } %>
                 ...
                 <%if (Convert.ToInt32(strOrderState) > 3)
                   { %>
                    <a href="javascript:void(0)" onclick="if(confirm('是否删除？')){location.href='?del=<%=strOrderNumber %>&sign=del'}"
                        ...
                        <%} %>
                        &nbsp;<a href="S_OrderDetail.aspx?guid=<%=strGuId %>&ordertype=<%=strOrderType %>"
                            class="alink_blue" target="_parent">查看</a>
                    <div>
                        <%if (dr["paymentname"].ToString() == "线下支付")
                          {
                              if (strOrderState == "0")
                              {%>
...
<div class="sp_dialog sp_dialog_out" style="display: none;" id="sp_dialog_outDiv">
    <div class="sp_dialog_cont" style="display: none;" id="sp_dialog_contDiv">
        ...
        <div style="width: 100%; display: none;" class="showpopone">
            <table cellspacing="0" cellpadding="0" border="0" style="padding-top: 10px; margin: 20px 0 20px 120px;">
                <tr class="up_spac">
                    <td align="right">
                        买家：
                    </td>
        ...
        <div style="width: 100%; display: none;" class="showpoptwo">
            <table cellspacing="0" cellpadding="0" border="0" style="padding-top: 10px; margin: 20px 0 20px 120px;">
                <tr class="up_spac">
                    <td align="right">
                        支付方式：
                ...
                <tr class="up_spac">
                    ...
                    <td>
                        <asp:LinkButton ID="btnSub" runat="server" Text="确认" class="chax btn_spc" Style="display: block;
                            color: #ffffff;"></asp:LinkButton>
