...
<script type="text/javascript" language="javascript">
       function check_confrim(){
            ...
            {
                $("span[lang='isnull']").show();return false;
            }
            else if($("#S_ProductCategory_Operate_ctl00_txtKeyWord").val().length>100)
            {
               $("#S_ProductCategory_Operate_ctl00_txtKeyWord").next().show();return false;
            }
            else if($("#S_ProductCategory_Operate_ctl00_txtDesc").val().length>200)
            {
                $("#S_ProductCategory_Operate_ctl00_txtDesc").next().show();return false;
            }
            else if(!IsNum($("#S_ProductCategory_Operate_ctl00_txtOrderId").val()))
            {
                $("#S_ProductCategory_Operate_ctl00_txtOrderId").next().show();return false;
            }
            return true;
       }
       function IsNum(num){var reNum=/^\d*$/; return(reNum.test(num));}
</script>
            ...
            <tr>
                ...
                <td class="bordrig">
                    <input type="text" id="txtName" runat="server" class="op_text" /><span class="star">*</span><span
                        lang="Isnull" style="color: Red; display: none;">分类名称不能为空或分类不能超过20个字符</span> <span lang="length_check"
                            style="color: Red; display: none;">最多20个字符</span>
                </td>
            </tr>
