<script>
    function funSelect(val,rguid)
    {
        if(val!="0")
        {
            if(confirm("本次开店需要支付预存款"+val+"元，是否继续开店！"))
            {   
                //判断预存款
                $.ajax({
                 type: "get",
 ...
 <div class="maijtitle">
    ...
    <ItemTemplate>
        <table border="0" cellspacing="0" cellpadding="0" class="buzlb">
          <tr>
            <td rowspan="4" style="color:#F60; font-weight:bold; width:130px;"><%#Eval("Name") %></td>
            <td style=" width:160px;">商品数：<%#Eval("MaxProductCount") %></td>
            <td rowspan="4">附加功能：<a href='S_ShopRankDetailed.aspx?type=open&guid=<%#Eval("Guid")%>' style=" color:blue">查看等级详细</a></td>
            <td rowspan="4">用户选择“默认等级”，可以立即开通</td>
          ...
          <tr>
            <td>模板数：<asp:Label ID="LabelShopTemplateCount" runat="server" Text='<%#Eval("shopTemplate")%>'></asp:Label></td>
            </tr>
          <tr>
            <td>收费标准：<%#Eval("price").ToString()=="0.00"?"免费": Eval("price").ToString()+"元/年"%></td>
            </tr>
          <tr>
            <td>需要审核： 是</td>
          </tr>
