 ...
 $(function(){  
         ...
         $("#productCategoryName").text($("#S_OpGoods1_ctl00_hidCategoryName").val());
          ...
          $.get(BrandAjaxUrl,null,function(data){
                    if(data!=""){
                        ...
                        $.each(vdata,function(m,n){
                             if(n.name!=$("#S_OpGoods2_ctl00_hidBrand").val().split(",")[1]){xhtml.push('<option value="'+n.guid+'">'+n.name+'</option>');}
                        });
                        if($("#S_OpGoods2_ctl00_hidBrand").val().split(",")[0]=="00000000-0000-0000-0000-000000000000")
                        {
                             ...
                             $("#OtherBrand").show();$("#txtPersonBrand").val($("#S_OpGoods2_ctl00_hidBrand").val().split(",")[1]);
                        }
          ...
          $(".select_Brand").change(function(){
                if($(this).val()=="00000000-0000-0000-0000-000000000000"){$("#OtherBrand").show(); }
                else{$("#OtherBrand").hide(); }
          });
          //选中操作控制
          $('span[spec_check="this_check"] > input[type="checkbox"]').click(function(){
                showEdit($(this).is(":checked"),this);
		        set_array();goods_stock_set();
	      });
        ...
        function showEdit(boolcheck,obj)
        {
            if(boolcheck){
                    ...
                    $("#fileup_"+$(obj).attr("spec_type")).show();
                        ...
                        if(i<2){$("#showUp_"+$(obj).attr("lang")).hide();}
                    }
        }
        // 按规格存储规格值数据
...
function price_total(){
	...
	if(max > min){$('input[name="goods_price_sum"]').val(number_format(min,2)+' - '+number_format(max,2));}
	else {$('input[name="goods_price_sum"]').val('');	}
	if(min != 10000000){$('#txtShopPrice').val(number_format(min,2));	}
}
// 计算商品库存
...
//设置规格库存配置html
 function goods_html_set(spec_bunch,td_1,td_2,td_3,checklen2,checklen3)
 {
    ...
    str +='<td><input class="spec_txt" type="text" name="spec['+spec_bunch+'][price]" onkeyup="checkpriceTxt(this);onchangestock(this)" maxlength="9" data_type="goods_price" value="0.00" spec_type="'+spec_bunch+'|price" value="" /></td>';
    str+='<td><input class="spec_txt" type="text" name="spec['+spec_bunch+'][stock]" onkeyup="NumTxt_Int(this);onchangestock(this)" maxlength="9" data_type="goods_stock" value="1" spec_type="'+spec_bunch+'|stock" value="" /></td>';
...
function goods_stock_set(){
	...
	for (var i_0=0; i_0<spec_group_checked[0].length; i_0++){
	     ...
	     var spec_bunch_1 = 'i_'+td_1[1];
	        if($('div[shop_spec="Spec_group_1"]').length=="0"){strApp.push(goods_html_set(spec_bunch_1,td_1,"","",spec_group_checked[1].length,spec_group_checked[2].length));}
         for (var i_1=0; i_1<spec_group_checked[1].length; i_1++){
         td_2 = spec_group_checked[1][i_1];
         var spec_bunch_2 = 'i_';
         spec_bunch_2 += td_1[1];spec_bunch_2 += td_2[1];
         ...
         var spec_bunch_3 = 'i_';
         spec_bunch_3 += td_1[1];spec_bunch_3 += td_2[1];spec_bunch_3+=td_3[1];
    ...
    function SpecfileUpload(obj,imgid)
    {
        ...
        $("#form1").ajaxSubmit({
            ...
            success: function(data, textStatus) {
                 ...
                 $("#errpicmsg").hide();
                 if(imageinfo.msg=="1"){    $(obj).next().val(imageinfo.imagepath);$(imgid).attr("src",imageinfo.imagepath);$(obj).val("");
    ...
    //分页代码  Jely liuxing
        function initMyPages(nowPage,pages){
           ...
           var endPage=pages;
            var startPage=1;
            if(pages>1){
                if(nowPage-4>0){startPage=nowPage-4;}
                if(nowPage+5<pages){endPage=nowPage+5;}else{endPage=pages;}
            }
            var pageNumStr="";
            if(nowPage!=1){
            ...
            for(var i=startPage;i<=endPage;i++){
                if(nowPage==i){pageNumStr+="&nbsp;<b><font color='red'>"+i+"</font></b>";}
                else{pageNumStr+="&nbsp;<a href='javascript:;' onclick='javascript:;GetAjaxContent("+i+")'>"+i+"</a>";}
            }
            //判断什么时候显示下一页
            if(nowPage<pages){
            ...
            $.get("/Api/S_OpGoods1.ashx?type=3&page="+index+"&str="+str+"&hidtype="+$("#albumtype").val(),null,function(data){
                      ...
                      if(data.split("|")[1]!=""){
                       ...
                       $("#albumlist ul").html(mylist.join(""));  
                        ...
                        $("#lblpageCount").html(count++);$("#lblpageIndex").text(index);$(".lblNumCount").text(countinfo.countinfo[0].allcount);  
            });
       }
       //从图库中调用商品图片
       function setpic(imgpath)
       {
           $("img[name='pro_img']").each(function(){
                if($(this).attr("src")=="/ImgUpload/noimg.jpg_100x100.jpg")
                {
                    ...
                    $(this).attr("src",imgpath);return false;
                }
           });
        ...
        //编辑器调用方法
        KindEditor.ready(function (K) {
             wapPaniceditor= K.create('.textwebeditvp', {
                //useContextmenu:false,
                //pasteType:0,
                //编辑器高度
                width: '100%',
                ...
                items: [
                'source', 'preview','plainpaste','wordpaste', 'fullscreen','clearhtml', 'removeformat', 'link', 'unlink', '|', 'about'
                ],afterBlur: function () { wapPaniceditor.sync(); }
            });
            prettyPrint();
        });