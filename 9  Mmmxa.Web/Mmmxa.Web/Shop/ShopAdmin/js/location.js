	...
	$.LocationSelect = {
		...
		build : function(user_opts) {
			...
			return $(this).each(function() {
				...
				specs.holder.append('<select name="province"><option value="0">请选择省市</option></select>&nbsp;&nbsp;城市：<select name="city" style="width:120px;"><option value="0">请选择城市</option></select>&nbsp;&nbsp;<select name="district" style="width:120px;"><option value="0">请选择区域</option></select><font color="red">*</font>');
		...
		init: function(specs){
		    ...
		    $.each(Json_vj,function(i){
			    ...
			    $(specs.selectors.province).change(function(evt){
			        fetch(evt.target);
			        // specs.selectors.district.html("");if(evt.target.value==0){specs.selectors.city.html("");}else{fetch(evt.target);}
			    });
			    specs.selectors.province.html("");   
