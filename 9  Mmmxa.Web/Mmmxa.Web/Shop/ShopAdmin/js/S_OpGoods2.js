  ...
  function checkprice(name)
    ...
    {
      setTimeout(function(){delayPrice(o)},2000);
    }
    function delayPrice(o)
    {
        var oo = /^\d{1,10}$|\.\w?$|^\d{1,10}\.\d{1,5}\w?$/;
        if(!oo.test(o.value)){o.value='0.00';}else{
    ...
    String.prototype.trim = function()
{
    return this.replace(/(^[\s]*)|([\s]*$)/g, "");
        ...
        function funsub()
        {
         ...
         {
            $("span[check='errorMsg']").hide();$("#stockSpec").hide();$("#showpic").hide();
            $("#S_OpGoods2_ctl00_hidPType").val("");
            $("input[name='pType']").each(function(){if($(this).attr("checked")){$("#S_OpGoods2_ctl00_hidPType").val($("#S_OpGoods2_ctl00_hidPType").val()+$(this).attr("id"));}});
            if($("#txtGoodsName").val().trim().length<3){$("#txtGoodsName").focus();$("#txtGoodsName").next().show();errorflag=1;window.scrollTo(0,200);}
            $("#S_OpGoods2_ctl00_hidName").val($("#txtGoodsName").val());
            if($("#txtStock").val().trim()==""||$("#txtStock").val().trim()=="0"){$("#txtStock").next().show();window.scrollTo(0,500);errorflag=2;}
            $("#S_OpGoods2_ctl00_hidRepertoryCount").val($("#txtStock").val());
            
            if($("#txtRepertoryAlertCount").val().trim()==""||$("#txtRepertoryAlertCount").val().trim()=="0"){$("#txtRepertoryAlertCount").next().show();window.scrollTo(0,500);errorflag=2;return false;}
            $("#S_OpGoods2_ctl00_hidRepertoryAlertCount").val($("#txtRepertoryAlertCount").val());
            if(!checkprice($("#txtShopPrice").val().trim())||$("#txtShopPrice").val().trim()==""||$("#txtShopPrice").val().trim()=="0.00"){$("#txtShopPrice").next().show();window.scrollTo(0,700);errorflag=3;}
            if(!checkprice($("#txtMarketPrice").val().trim())||$("#txtMarketPrice").val().trim()==""){$("#txtMarketPrice").next().show();window.scrollTo(0,800);errorflag=4;}
            $("#S_OpGoods2_ctl00_hidShopPrice").val($("#txtShopPrice").val());
            $("#S_OpGoods2_ctl00_hidMarketPrice").val($("#txtMarketPrice").val());
            ...
            //是否是虚拟物品
            $("input[name='IsReal']").each(function(){ if($(this).is(":checked")){$("#S_OpGoods2_ctl00_hidIsReal").val($(this).val());}  });
            var $selectbrand=$(".select_Brand").find("option:selected");
            if($selectbrand.text()=="其它"){$("#S_OpGoods2_ctl00_hidBrand").val($selectbrand.val()+","+$("#txtPersonBrand").val());}
            else if($selectbrand.val()!="0"){$("#S_OpGoods2_ctl00_hidBrand").val($selectbrand.val()+","+$selectbrand.text());}
            ...
            $("input[name='check_spec']").each(function(){
                    if(!$(this).is(":checked")&&$(this).attr("vd")!=""){arrvd.push($(this).attr("vd")); }
            });
            $("#S_OpGoods2_ctl00_hidVd").val(arrvd.join(","));
            var spec_stock=new Array();var spec_stock_value=new Array();
            var bflag=true,bflag2=true,bflag3=true;var baseid=null,strV=null;
            var objthis;
            $('#Spec_body tr').find("input").each(function(){
                   if($(this).attr("name")=="spec[sp_value]"){strV=null;bflag2=true;
                        spec_stock.push($(this).attr("value").replace("*","x")+","+$(this).attr("sp_id"));
                        if(bflag){ baseid=$(this).attr("spec_bunch");bflag=false;}
                   }
                   else{
                        if(bflag2){
                            strV=baseid;baseid=null;
                            strV=spec_stock.join("|")+"*"+strV;
                            spec_stock=new Array();bflag=true;bflag2=false;
                        }
                        if($(this).attr("data_type")=="goods_price"){
                        if($(this).val()==""||$(this).val()=="0.00"){objthis=$(this);bflag3=false;return false;}
                        strV+=","+$(this).val();}
                        if($(this).attr("data_type")=="goods_stock"){
                        if($(this).val()==""){objthis=$(this);bflag3=false;return false;}
                        strV+=","+$(this).val();}
                        if($(this).attr("data_type")=="goods_no"){strV+=","+$(this).val();spec_stock_value.push(strV);}
                       
                   }
            });
            if(!bflag3)
            {
                 objthis.focus();objthis.addClass("errortxt");errorflag=5;window.scrollTo(0,600);
                 $("#stockSpec").show();
                 return bflag3;
            }
            $("#S_OpGoods2_ctl00_hidBaseStock").val(spec_stock_value.join("&"));//取得库存值 
             ...
             $(".yansda .spec").each(function(){
                     ...
                     $(this).find("input[name='check_spec']").each(function(){
                            ...
                            {
                               icheckv++;isflag=false;
                            }
                     });
             });
             if(icheckv!=checklen&&icheckv!=0)
             {
                 alert("规格组必须选择！");return false;
             }
            ...
            $("#S_OpGoods1_ctl00_Prop_show .prop_radio_text").each(function(){
                    if($(this).is(":checked")){baseprop.push($(this).attr("prop_type"));}
            });
            ...
            var checkidarry=new Array(); var checknamearry=new Array();
             var checkshoptype=true;
             $("input[name='shopCat']").each(function(){
                     if($(this).is(":checked")){   checkshoptype=false;checkidarry.push($(this).val());checknamearry.push($(this).next().text());}
             });
             if(checkshoptype){$(".xiangxsp_nr").find("span[check='errorMsg']").show();errorflag=7;window.scrollTo(0,2400);}
            /*提交时候操作属性相关方法*/
            ...
            if(isshowImg)
            {
                $(".select_Brand").focus(); $("#showpic").show();errorflag=9;
            }
            $("#S_OpGoods2_ctl00_hidproImage").val($("#imgvj_1").attr("src"));
            var pro_imgap=new Array();
            $("img[name='pro_img']").each(function(){if($(this).attr("src").indexOf("noimg")==-1){pro_imgap.push($(this).attr("src"));}  });
             $("#S_OpGoods2_ctl00_hidpro_img").val(pro_imgap.join(","));
             /*操作商品图片方法*/
             var info = $("#p_local").getLocation();
             if(info.pcode=="0")
             {
                 $("#p_local").next().show();window.scrollTo(0,2100);errorflag=6;
             }
             ...
             //邮费模板不能为空验证
             if($("#S_OpGoods2_ctl00_hidsubfee").val()=="0"&&$("#S_OpGoods2_ctl00_hidfee_template").val()==""&&$("#S_OpGoods2_ctl00_hidfee").val()=="0"){$("#fee_check").show();window.scrollTo(0,2200);errorflag=8;}
             $("#spanload").show();
             ...
             {
                  $("#ftime").show();errorflag=10;
             }
             
             if($("#S_OpGoods2_ctl00_txtWapDetail").text().length>4000)
             {
                $("#wapdetailfont").css("color","red");window.scrollTo(0,2000);return false;
             }
             ...
             if(errorflag!=0)
             {
                switch(errorflag)
                {
                   case "1":window.scrollTo(0,150);return false;break;
                   case "2":window.scrollTo(0,500);return false;break;
                   case "3":window.scrollTo(0,700);return false;break;
                   case "4":window.scrollTo(0,800);return false;break;
                   case "5":window.scrollTo(0,600);return false;break;
                   case "6":window.scrollTo(0,2100);return false;break;
                   case "7":window.scrollTo(0,2200);return false;break;
                   case "8":window.scrollTo(0,2200);return false;break;
                   case "9":window.scrollTo(0,2200);return false;break;
                   case "10":window.scrollTo(0,2400);return false;break;
                }
                return false;
             }
     ...
     String.prototype.replaceAll = function(reallyDo, replaceWith, ignoreCase) {  
        ...
        //设置商品相关的参数
        $(function(){
              $("tr[name='spec_Detail']").each(function(){
                    var SpecDetail=$(this).attr("value");
                    ...
                    //$("#Spec_body tr").prepend(SpecNewArry.join(""));
                    $(this).prepend(SpecNewArry.join(""));
              });
             $("input[name='IsReal']").click(function(){
                    if($(this).val()=="0")
                    {
                         ...
                         $("input[name='fee_check']").each(function(m,n){n.disabled=false;});
                   }
             });
               //设置宝贝类型隐藏域
...
//              });
                ...
                $("input[name='fee_check']").click(function(){$("#S_OpGoods2_ctl00_hidfee").val($(this).val());
                ...
                $("input[name='sub_fee_check']").click(function(){
                        ...
                        if($(this).val()=="0"){$("#subfee").show();}else{$("#subfee").hide();}
                });
             ...
             $("input[name='publishGoods']").click(function(){
                ...
                if($(this).val()=="1"){$("#S_OpGoods2_ctl00_txtSendTime").removeAttr("disabled");}
             });
            ...
            $("input[name='isPanic']").change(function(){$("#S_OpGoods2_ctl00_hidState").val($(this).val());
                    if($(this).val()=="1"){$("#spanendtime").show();$(".showPanic").show();window.scrollTo(0,2100);
                          if(tipAdd[1]=="0"){
                           $("#btnOk").attr("disabled","disabled");
                           $("#erroradd").show();
                         }else{
                    ...
                    }else{$("#spanendtime").hide();$(".showPanic").hide();}
             });
             ...
             $("input[name='file_btn']").click(function(){
                    if(confirm("是否删除？"))
                    {
                        ...
                        $this.parent().hide().next().show();return false;
...
function  SelectShopPostage(obj)
{
   if(lock==0)
   {
     ...
     { 
        ...
        $(obj).val("重新选择模板");$(obj).prev().text(tArray[1]);
...
$(function(){
        ...
        $("input[name='IsReal']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidIsReal").val()){$(this).click();}
        });
        $("input[name='gType']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidGType").val()){$(this).click();}
        });
        $("input[name='pType']").each(function(){
                if($("#S_OpGoods2_ctl00_hidPType").val()!=""){
                     if($("#S_OpGoods2_ctl00_hidPType").val().indexOf($(this).attr("id"))!=-1){$(this).attr("checked","checked");}
                }
        });
        ...
        $("#txtShopPrice").val($("#S_OpGoods2_ctl00_hidShopPrice").val().replace(",",""));
        $("#txtMarketPrice").val($("#S_OpGoods2_ctl00_hidMarketPrice").val().replace(",",""));
        ...
        if($("#S_OpGoods2_ctl00_hidpro_img").val()!=""){
                    ...
                    {
                        $("#imgvj_1").attr("src",$("#S_OpGoods2_ctl00_hidpro_img").val());$("#imgvj_1").removeAttr("onerror");
                    }
        }
        ...
        if($("#S_OpGoods2_ctl00_hidarea").val()!=""){
            ...
            if(area[1]!=""){
                if(areaname[0]!=""||areaname[0]!="undefined"){$("select[name='province']").append("<option value=\""+areacode[0]+"\" selected=\"selected\">"+areaname[0]+"</option>");}
                if(areaname[1]!=""||areaname[1]!="undefined"){ $("select[name='city']").append("<option value=\""+areacode[1]+"\" selected=\"selected\">"+areaname[1]+"</option>");}
                if(areaname[2]!=""||areaname[2]!="undefined"){ $("select[name='district']").append("<option value=\""+areacode[2]+"\" selected=\"selected\">"+areaname[2]+"</option>");}
            }
        }
        $("input[name='fee_check']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidfee").val()){$(this).click();}
        });
        $("input[name='sub_fee_check']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidsubfee").val()){$(this).click();}
        });
        ...
        $("input[name='publishGoods']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidpublishGoods").val()){$(this).click();}
        });
        $("input[name='setStock']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidsubStock").val()){$(this).click();}
        });
        
         $("input[name='isPanic']").each(function(){
                if($(this).val()==$("#S_OpGoods2_ctl00_hidState").val()){$(this).click();
                        if($(this).val()=="1"){$("#spanendtime").show();$(".showPanic").show();window.scrollTo(0,2200);}
                }
        });
