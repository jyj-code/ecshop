 $(function(){
            $(".cancelActivity").click(function(){
                    ...
                    $(this).hide();$(this).parent().prev().text("已取消");
...
$.get("/Api/ShopAdmin/S_Limit_ProductOp.ashx?guid="+guid+"&lid="+lid+"&discount="+discount+"&type=addlimitproduct",null,function(data){
           ...
           $(".btn_save").click(function(){
                ...
                {
                    $(this).parent().next().find(".ord_sub").css("color","red").text("折扣值不合法！");return false;
                }
...
function checksub()
            {
               ...
               if(name==""){
                    $("#S_Limit_ActivityOpreate_ctl00_txtName").focus().next().show().text("活动名称不能为空！");return false;
               }
               if(!compareTime($("#showstarttime").text(),starttime))
               {
                    $("#S_Limit_ActivityOpreate_ctl00_txtStartTime").focus().next().show().text("开始时间必须大于"+$("#showstarttime").text()+"！");return false;
               }
               if(!compareTime(starttime,endtime))
               {
                    $("#S_Limit_ActivityOpreate_ctl00_txtEndTime").focus().next().show().text("开始时间不能大于结束时间！");return false;
               }
...
//               }
               if(!checkDisCount(DisCount)){
                    $("#S_Limit_ActivityOpreate_ctl00_txtDisCount").focus().next().show().text("折扣不合法！");return false;
               }
               return true;
            }
