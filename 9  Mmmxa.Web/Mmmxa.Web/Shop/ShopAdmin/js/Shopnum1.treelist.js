       ...
       if($("#sub_two_"+parentid).html()==""){
           $.get(ajaxurl+"?parentid="+parentid+"",null,function(data){
                   ...
                   $.each(typedata,function(m,n){
                         arry.push(treelist_two_html(n.id,n.name,n.orderid,n.isshow,n.v,parentid));
                   }); 
                   arry.push('</table></td>');
...
//    }
    ...
    function treelist_save(o)
    {
         var adata=$(o).attr("lang")+"|"+$(o).parent().prev().prev().find("input[name='orderby']").val()+"|"+$(o).parent().prev().prev().find("input[name='orderby']").parent().next().children("input").val();
          ...
          alert("保存成功！");
          var xv=$(o).parent().prev().prev().find("input[name='orderby']").parent().next().children("input").val();
          $(o).parent().prev().prev().find("input[name='orderby']").parent().next().children("input").val(xv);
          window.location.href="S_ProductCategory.aspx?r="+Math.random();
    }
    //删除
                   ...
                   $.get(ajaxurl+"?delectid="+$this.attr("lang"),null,function(data){
                      if(data.split("|")[0]!="ok"){
                           ...
                           alert("删除成功,部分分类不能删除！");
                             ...
                             if ($this.parent().parent().attr("id").indexOf("parent") == -1) {
         ...
         $(".cporder").click(function(){
             ...
             $(".vcheck").each(function(m,n){
                  if($(this).attr("checked")){ 
                      append.push($(this).val()+"|"+$(this).parent().next().next().find("input").val()+"|"+$(this).parent().next().next().next().find("input").val());
                  }
             });
