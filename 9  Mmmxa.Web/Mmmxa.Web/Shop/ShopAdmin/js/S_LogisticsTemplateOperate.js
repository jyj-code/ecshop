...
String.prototype.trim = function()
{
    return this.replace(/(^[\s]*)|([\s]*$)/g, "");
     ...
     var istemplatename=true;//模板名称是否可用
      ///检查模板名称
    ...
    ///快递添加地区组
       ...
       function SetDivPosition(vobj,x,y,logisticname)
       {
            ...
            if(logisticname=="express")
            {
                $("#ExpressRows tr").each(function(){
                        ...
                        }else{MyCheck.push($(this).find("td:eq(0) input:eq(1)").val());}
                });
            }
      ...
      {
          $("#alldiv").css('display','none'); $('.ks-ext-mask').css('display','none');$("#dialog_areas").css('display','none');
      }
      
     var vselspan=null;//触发地区 里面的span
     var vselhidden=null;//触发地区 里面hidden
     var vselregionnames=null;//触发地区名称 里面的hidden
     $(function(){
         /*	关闭弹出的市级小层*/
...
/*	省份下拉事件*/
    ...
    function checkprice(name)
    {
      ...
      var priceyz=/^(([0-9]{1,3})|([0-9]{1,3}\\.[0-9]{0,2}))$/;  //正则表达式
