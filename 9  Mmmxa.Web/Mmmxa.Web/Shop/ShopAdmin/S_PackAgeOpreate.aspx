           ...
           var endPage=pages;
            ...
            $(function(){
                   $("#addProduct").click(function(){
                         $("#showProuct,#search_Pro").show();LoadCategoryAjax(); GetAjaxContent("1");
                   }); 
                   ...
                   $("#loadProduct #btnAdd").live("click",function(data){
                        ...
                        if($(this).val()=="取消搭配")
                        {
                            $("#trd_"+id).remove();$(this).val("添加搭配");
                        }
                        else
                        {
                             if($("#tbProduct tr").size()>4)
                             {
                                alert("您已经添加了5件商品了!");return false;
                             }
                             ...
                             $("#tbProduct").append(xhtml.join(""));totalPrice();
                        }
                   });
                   $("#tbProduct #btnAdd").live("click",function(data){
                         if(confirm("是否移除？"))
                         {
                             ...
                             var packId='<%=Common.ReqStr("packid") %>';
            ...
            function totalPrice()
            {
               ...
               $("#S_PackAgeOpreate_ctl00_lblShopPrice").text(pricecount);
            }
            function GetAjaxContent(pageid)
            {
               var tp=$("#txtProductName").val();var pcode=$("#ProSelect").html();
               ...
               $.get("/Api/ShopAdmin/S_PackAgeOpreate.ashx?packId="+packId+"&pageid="+pageid+"&pcode="+pcode+"&pname="+escape(tp)+"",null,function(data){
                            ...
                            if(data!=""){
                             ...
                             $.each(vdata,function(m,n){
                                    ...
                                    xhtml.push("<td><input type=\"hidden\" value=\""+n.shopprice+"\" name=\"hid_price\"/>"+n.shopprice+"</td>");
                                    xhtml.push("<td>"+n.repertorycount+"</td>");
                            ...
                            {
                                $("#loadProduct .addpack_"+arryPackId[i]).val("取消搭配");$("#tbProduct .addpack_"+arryPackId[i]).parent().parent().attr("id","trd_"+arryPackId[i]);
                            }
                }); 
            }
        ...
        KindEditor.ready(function (K) {
             editor= K.create('.textwebedit', {
                ...
                'plainpaste', 'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright', '|', 'fullscreen',
                 'forecolor', 'hilitecolor', 'bold', '/','formatblock', 'fontname', 'fontsize', 'lineheight', 'removeformat', '|', 'image', 'multiimage',
...
function number_format(num, ext){
    ...
    return _str;
        ...
        function checksub()
        {
            ...
            {
               alert("活动名称不能为空！");$("#S_PackAgeOpreate_ctl00_txtName").focus();return false;
            }
            var checkguid=new Array();var Numflag=1;
            $("#tbProduct input[name='hidguid']").each(function(){
                    checkguid.push($(this).val());Numflag++;
            });
            if(!checkprice($("#S_PackAgeOpreate_ctl00_txtSalePrice").val())||$("#S_PackAgeOpreate_ctl00_txtSalePrice").val()=="")
            {
                alert("价格不合法！");$("#S_PackAgeOpreate_ctl00_txtSalePrice").focus();return false;
            }
            if(Numflag<2)
            {
              alert("至少选择两条数据进行套餐搭配！");return false;
            }
