...
<ShopNum1:SupplyDemandAds ID="SupplyDemandAds" runat="server" SkinFilename="SupplyDemandAds.ascx" AdvID="ad2" />
        ...
        <div class="supply_demand_right_middle_leftlist fl">
            <ul>
                <asp:Repeater ID="RepeaterData1" runat="server">
                    <ItemTemplate>
                        <li>· <a href='<%#ShopUrlOperate.RetUrl("SupplyDemandDetail",((DataRowView)Container.DataItem).Row["Guid"])%>' target="_blank" title='<%#((DataRowView)Container.DataItem).Row["Title"] %>'>
        ...
        <div class="supply_demand_right_middle_leftlist fr">
            <ul>
                <asp:Repeater ID="RepeaterData2" runat="server">
                    <ItemTemplate>
                        <li>·<a href='<%#ShopUrlOperate.RetUrl("SupplyDemandDetail",((DataRowView)Container.DataItem).Row["Guid"])%>' target="_blank" title='<%#((DataRowView)Container.DataItem).Row["Title"] %>'><%# ShopNum1.Common.Utils.GetUnicodeSubString(((DataRowView)Container.DataItem).Row["Title"].ToString(),32,"")%></a></li>
                    </ItemTemplate>
                </asp:Repeater>
            </ul>
        </div>
            ...
            <div class="supply_demand_right_middle_leftlist fl" style="width: 200px;">
                <ul>
                    <asp:Repeater ID="RepeaterData3" runat="server">
                        <ItemTemplate>
                            <li>·<a href='<%#ShopUrlOperate.RetUrl("SupplyDemandDetail",((DataRowView)Container.DataItem).Row["Guid"])%>' target="_blank" title='<%# ((DataRowView)Container.DataItem).Row["Title"] %>'><%# ShopNum1.Common.Utils.GetUnicodeSubString(((DataRowView)Container.DataItem).Row["Title"].ToString(),30,"")%></a></li>
                        </ItemTemplate>
                    </asp:Repeater>
                </ul>
            </div>
