        ...
        <ul>
            <asp:Repeater ID="RepeaterData" runat="server">
                <ItemTemplate>
                    <li>
                        <div class="art_lists">
                            <div class="art_imgs"><span><%# ((DataRowView)Container.DataItem).Row["ClickCount"]%></span></div>
                            <div class="art_content">                                
                                <h5>
                                    <a  href="<%# ShopUrlOperate.RetUrlNew("ArticleDetail",((DataRowView)Container.DataItem).Row["Guid"]) %>">
                                    <%# ((DataRowView)Container.DataItem).Row["Title"]%></a>
                                </h5>                                
                                <a href="<%# ShopUrlOperate.RetUrlNew("ArticleDetail",((DataRowView)Container.DataItem).Row["Guid"]) %>">[详情]</a> 
                                <p>
                                    分类：<%# ((DataRowView)Container.DataItem).Row["Name"]%> 来源：<%# ((DataRowView)Container.DataItem).Row["Publisher"]%> 发布时间：<%# Convert.ToDateTime(((DataRowView)Container.DataItem).Row["CreateTime"]).ToString("yyyy-MM-dd") %></p>
                            </div>                            
                        </div>
                    </li>
                </ItemTemplate>
