...
<div id="ThemeEnd" style="display:none; text-align:center; font-family:微软雅黑; font-size:20px; font-weight: bold;color:#FF6600;">活动已结束！</div>
<div class="ThemeAdv" id="ThemeImages">
    <asp:Image ID="ImageTheme" runat="server" Width="1190px" Visible=false/>
</div>

<div id="IsShowNoData" style="display:none;">
    <script type="text/javascript" language="javascript">
            $(function(){
                    if($("input[name='CheckNoData']").size()==0)
                    {
                       $("#IsShowNoData").show();
                    }
            });
...
<div class="ThemeBox clearfix" id="ThemeProducts">
    <asp:Repeater ID="RepeaterData" runat="server">
        <ItemTemplate><input type="hidden" name="CheckNoData" />
            <%# (Container.ItemIndex+1)%5==0?"<div class=\"ThemeList LastList\">":"<div class=\"ThemeList\">" %>
            <div class="ThemeTitle">
            </div>
            <div class="ThemeImg">
                <a href='<%#ShopUrlOperate.shopHrefNew(((DataRowView)Container.DataItem).Row["ProductGuid"].ToString(),((DataRowView)Container.DataItem).Row["MemLoginID"].ToString() ) %>'
                    target="_blank" title='<%# ((DataRowView)Container.DataItem).Row["ProductName"]%>'>
                    <img src='<%# Eval("ProductImage")%>' onerror="javascript:this.src='/ImgUpload/noimg.jpg_160x160.jpg'" /></a>
            </div>
            <div class="ThemeInfo">
                <div class="clearfix">
                    <span class="curPrice fl">现价:￥<i><%#Eval("ProductPrice")%></i></span> <span class="oriPrice fr"><i>原价</i>:<%#Eval("MarketPrice")%></span>
                </div>
                <div class="intro">
                    <a href='<%#ShopUrlOperate.shopHrefNew(((DataRowView)Container.DataItem).Row["ProductGuid"].ToString(),((DataRowView)Container.DataItem).Row["MemLoginID"].ToString() ) %>'
                        target="_blank" title='<%# ((DataRowView)Container.DataItem).Row["ProductName"]%>'>
