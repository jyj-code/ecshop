        ...
        <asp:Repeater ID="RepeaterSupplyFirst" runat="server">
            <ItemTemplate>
                <div class="mess_conts">
                    <a href='<%# ShopUrlOperate.RetUrlNew1("SupplyDemandDetail",((DataRowView)Container.DataItem).Row["ID"].ToString(),"guid") %>'
                        class="messpic">
                        <img src='<%# Eval("Image")%>'
                            width="95" height="75" onerror="javascript:this.src='../ImgUpload/noImage.gif'"  runat="server"/></a>
                    <div class="messname">
                        <p>
                            <a href='<%# ShopUrlOperate.RetUrlNew1("SupplyDemandDetail",((DataRowView)Container.DataItem).Row["ID"].ToString(),"guid") %>'
                                title='<%# Eval("Title")%>'>
