...
<div id="divAgainLogin" runat="server" class="PayPage">
    ...
    <div class="PayPageInfo">
        <p>交易号：<asp:TextBox ID="TextBoxOrderID" runat="server" CssClass="font1"></asp:TextBox></p>
        <p>交易金额：<asp:Label ID="LabelShouldPay" runat="server" CssClass="font2"></asp:Label>元</p>
        <p class="fenge"><b>预存款账户</b>可交易余额：<asp:Label ID="LabelAdvancePayment" runat="server" CssClass="font3"></asp:Label>元</p>
        <p class="fenge" runat="server" id="noPay"><b> <a target="_blank" href='http://<%=ShopNum1.Common.ShopSettings.siteDomain%>/main/account/A_Index.aspx?toaurl=A_PwdSer.aspx'>警告：您尚未设置支付密码，马上去设置支付密码</a></b></p>
        <div class="buzhu" id="xianshi" style="display:none;">
            <p>您的账户没有可交易余额不足，请使用其它方式付款，或<a href="http://<%=ShopNum1.Common.ShopSettings.siteDomain%>/main/account/A_Index.aspx?toaurl=A_AdPayRecharge.aspx" target="_blank">充值</a>后付款</p>
        </div>
        ...
        <p><asp:Button ID="ButtonPay" class="PaySure" runat="server" Text="确定支付" OnClientClick="return checkPay(this)" />
        <img class="append_image" alt="正在加载请稍等候" src="/Main/Themes/Skin_Default/Images/lodding.gif" style="display:none;" /></p>       
