...
function KeyEnter(o)
{
    ...
    function searchgo(c_type,c_name){
        ...
        var name=encodeURIComponent($("#textfield").val().replaceAll("/","%2F").replaceAll(" ",""));
             ...
             var changetag="<p class=\"ll_p\" onclick=\"chang(1)\"><a >搜索“"+str+"” <span>商品</span></a></p>";
                ...
                success: function(result) {
                  if(result!="0")
                  {
                        ...
                        $("body").append("<div id=\"s"+i+"\">"+ajaxdata.join("")+"</div>");
                        $(".checktag p").show();
                        $(".ll_all_search p:eq("+var_tag+")").hide();
                   }
                  else
                  {
                     ...
                     $("body").append("<div id=\"s"+i+"\"><div style=\"line-height:2\">没有搜索到["+str+"]符合条件数据</div></div>");
                  }
                },
