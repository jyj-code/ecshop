<%@ WebHandler Language="C#" Class="S_AdminOpt" %>
...
public class S_AdminOpt : IHttpHandler
{
    ...
    public string AdvancePayment()
    {
        ...
        string money = Common.GetNameById("AdvancePayment", "ShopNum1_Member", "  AND  MemLoginID='" + MemberLoginID + "'");
    ...
    {
        #region 获得当前的预存款
        string guid = Common.GetNameById("Guid", "ShopNum1_ShopInfo", "  AND  ShopName='" + shopName + "'");
    ...
    {
        string guid = Common.GetNameById("IdentityCard", "ShopNum1_ShopInfo", "  AND  IdentityCard='" + IdentityCard + "'");
        if (!string.IsNullOrEmpty(guid))
        {
            return "true";
        }
    ...
    {
        //在域名申请表中查询 是否有相同域名
        string DoMain = Common.GetNameById("DoMain", "ShopNum1_ShopURLManage", "  AND  DoMain='" + ShopUrl + "'");
        if (!string.IsNullOrEmpty(DoMain))
        {
            return "true";
        }
        //在店铺表中查询 是否有相同的域名
        string DoMain1 = Common.GetNameById("Guid", "ShopNum1_ShopInfo", "  AND  ShopUrl='" + ShopUrl + "'");
    ...
    public string CheckPayPwd(string payPwd)
    {
        ...
        //会员登录ID
        string MemberLoginID = decodedCookieShopMemberLogin.Values["MemLoginID"].ToString();
        string payPwd1 = Common.GetNameById("PayPwd", "ShopNum1_Member", "  AND  MemLoginID='" + MemberLoginID + "'");
        if (Encryption.GetMd5SecondHash(payPwd) == payPwd1)
        {
            return "true";
        }
        return "false";
