<%@ WebHandler Language="C#" Class="api_reply" %>
...
public class api_reply : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (type.ToLower())
        {
            //操作文字回复
            case "op_replytxt":
                string op_keys = HttpUtility.UrlDecode(Operator.FilterString(context.Request["keys"] ?? string.Empty), System.Text.Encoding.GetEncoding("gb2312"));
                string op_msgtype = context.Request["msgtype"] ?? string.Empty;
                string op_content = HttpUtility.UrlDecode(Operator.FilterString(context.Request["content"] ?? string.Empty), System.Text.Encoding.GetEncoding("gb2312"));

                op_content = op_content.Replace("<", "&lt;").Replace(">", "&gt;");
                string strUrlx=string.Empty;
                if (op_content.IndexOf("&") != -1 && op_content.IndexOf("http") != -1)
                {
                    string[] opArray = op_content.Split('&');
                    if (opArray.Length >= 3)
                        strUrlx = opArray[3];
                    op_content = op_content.Replace(strUrlx, HttpUtility.UrlEncode(strUrlx, Encoding.GetEncoding("gb2312")));
                }
                else
                {
                    strUrlx = op_content;
                }
                op_content = op_content.Replace("%22","\"");
                string op_ruleid = context.Request["ruleid"] ?? string.Empty;



                //构建规则实体
                ShopNum1_Weixin_ReplyRule rulemodel = new ShopNum1_Weixin_ReplyRule();
                ...
                //构建关键字实体
                List<ShopNum1_Weixin_ReplyRuleKey> rulekeylist = new List<ShopNum1_Weixin_ReplyRuleKey>();
                ...
                //构建回复内容实体
                ShopNum1_Weixin_ReplyRuleContent contentmodel = new ShopNum1_Weixin_ReplyRuleContent();
                contentmodel.RepMsgContent = op_content;



                //检查关键字是否已存在 添加操作时 验证关键字是否存在
                if (!shopnum1_weixin_replyrule_active.KeyExists(shopmemloginid, StringHelper.RemoveLast(keys.ToString()), op_ruleid))
                {
            ...
            case "op_replyimgtxt":
                ...
                if (opimgtxt_msgtype == "1")
                    opimgtxt_articlecontent = HttpUtility.UrlEncode(opimgtxt_articlecontent, Encoding.GetEncoding("gb2312"));
                ...
                //检查关键字是否已存在 添加操作时 验证关键字是否存在
                if (!shopnum1_weixin_replyrule_active.KeyExists(shopmemloginid, StringHelper.RemoveLast(keys.ToString()), opimgtxt_ruleid))
                {

                    //构建图片链接文章实体
                    ShopNum1_Weixin_ReplyRuleContentArticle articlemodel = new ShopNum1_Weixin_ReplyRuleContentArticle();
                    ...
                    if (shopnum1_weixin_replyrule_active.InsertRule(rulemodel, rulekeylist, contentmodel, articlemodel, opimgtxt_chirldids))
                    {
                        context.Response.Write("{\"errcode\":0,\"errmsg\":\"操作成功!\"}");
                    }
                    else
            ...
            case "get_allcontent":
                ...
                DataTable dt = shopnum1_weixin_replyrule_active.SelectReplyRule(shopmemloginid, (int)MsgType.TxtImgMsg, get_ruleid);
                if (dt.Rows.Count != 0)
                {
    ...
    {
        List<ShopNum1_Weixin_ReplyRuleContent> contentlist = new List<ShopNum1_Weixin_ReplyRuleContent>();
