...
public class api_weixin : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (!string.IsNullOrEmpty(ShopOwner))
        {
            ...
            System.IO.File.AppendAllText(context.Server.MapPath("~/log/gettoken.txt"), ShopOwner + "|" + Token);
            ...
            if (!string.IsNullOrEmpty(Token))
            {
                if (context.Request.HttpMethod.ToLower() == "post")
                {
                    ...
                    if (!string.IsNullOrEmpty(postStr))
                    {
                        ...
                        System.IO.File.AppendAllText(context.Server.MapPath("~/log/getpostStr.txt"),postStr);
                        WriteLogFile(result);
                        //回复消息
    ...
    public string RepMsg_Content(string postStr, DataTable dt)
    {
        ...
        System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/getRepMsg.txt"), requestXML.ToUserName + "|" + requestXML.FromUserName + "|" + requestXML.CreateTime + "|" + requestXML.MsgType + "|" + requestXML.Title + "|" + requestXML.Description + "|" + requestXML.Url + "|" + requestXML.MsgId);
        //根据返回的xml消息类型，解析xml
        ...
        try
        {
            switch (requestXML.MsgType)
            {
                ...
                case "text":
                    ...
                    System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/getReturnVal1.txt"), key+"xxxxxxxxxxxxxxxxx"+ReturnVal);
                    //如果关键子不存在，查询无关键字回复
                    if (string.IsNullOrEmpty(ReturnVal))
                    {
                        ...
                        if (Convert.ToBoolean(dt.Rows[0]["IsOpenNotFindKey"]))
                        {
                            ...
                            ReturnVal = KeyContent(dt, key, msgxml, username);
                            System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/getReturnVal2.txt"), key + "xxxxxxxxxxxxxxxxx" + ReturnVal);
                        }
                    }
                ...
                case "event":
                    ...
                    switch (requestXML.Wxevent)
                    {
                        ...
                        case "subscribe":
                            ...
                            try
                            {
                                shopnum1_weixin_shopmember_active.AddWeiXinMember(dt.Rows[0]["ShopMemLoginId"].ToString(), requestXML.FromUserName, requestXML.ToUserName, userinfo);
                            }
                            catch (Exception ex)
                            {
                                WriteLogFile(ex.Message);
        ...
        if (ruleids.Rows.Count != 0)
        {
            ...
            if (contentdt.Rows.Count != 0)
            {
                ...
                if (Convert.ToInt32(ruleids.Rows[index]["RepMsgType"]) == (int)MsgType.TxtMsg)
                {
                    ...
                    //如果是微信商城连接
                    Msg = Msg.Replace("{0}", username).Replace("{1}", shopid);
                    
                    ReturnVal = msgxml.ReplyTxt(Msg);
                    System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/gettxtMsg.txt"), Msg + "|x|||||||||||||||||x|" + ReturnVal);
                    WriteLogFile(Msg);
                }
                else
                {
                    ...
                    System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/gettcontentdt1.txt"), replyinfomodel.Title + "|x|x|x|x|" + replyinfomodel.Description + "|x|x|x|x|" + replyinfomodel.ImgSrc + "|x|x|x|x|" + ruleids.Rows[index]["ruleid"]);
                    //获取详细内容
                    DataTable artidt = shopnum1_weixin_replyrule_active.SelectArticle(ruleids.Rows[index]["ruleid"].ToString());
                    if (artidt.Rows.Count != 0)
                    {
                        if (Convert.ToInt32(artidt.Rows[0]["Type"]) == (int)ArticleType.link)
                        {

                            replyinfomodel.Url = string.Format(ConfigurationManager.AppSettings["wap_urlhost"].ToString(), username, shopid, HttpUtility.UrlEncode(artidt.Rows[0]["ArticleContent"].ToString()));
                        }
                        else
                        {
                            replyinfomodel.Url = string.Format("http://{0}/Shop/ShopAdmin/W_ShopArticle.aspx?id={1}", HttpContext.Current.Request.Url.Host, artidt.Rows[0]["ID"].ToString());
                    ...
                    //        }
                    //        System.IO.File.AppendAllText(System.Web.HttpContext.Current.Server.MapPath("/log/" + System.DateTime.Now.ToString("yyyyMMdd") + "_Apayx.txt"), replyinfomodel.Url);
                    //    }
                    ...
                    System.IO.File.AppendAllText(HttpContext.Current.Server.MapPath("~/log/gettcontentdt2.txt"), ReturnVal);
                }
            }
        }
    ...
    private string GetAccess_token(string shopmemloginid)
    {
        ...
        System.IO.File.AppendAllText(System.Web.HttpContext.Current.Server.MapPath("/log/" + System.DateTime.Now.ToString("yyyyMMdd") + "_Apay.txt"), url);
