<%@ WebHandler Language="C#" Class="api_menu" %>
...
public class api_menu : IHttpHandler
{
        ...
        switch (type.ToLower())
        {
            case "menu_add":
                ...
                if (menus != null)
                {
                    ...
                    {
                        menu.Name = Operator.FilterString(HttpUtility.UrlDecode(menu.Name, Encoding.UTF8));
                        menu.Value = Operator.FilterString(menu.Value);

                        foreach (MenuButton item in menu.SubButton)
                        {
                            item.Name = Operator.FilterString(HttpUtility.UrlDecode(item.Name, Encoding.UTF8));
    ...
    public string Get_MenuJson(string shopmemloginid)
    {
        ...
        List<MenuButton> menu_list = new List<MenuButton>();
        MenuButton menu;
        ...
        foreach (DataRow dr in menu_dt.Select("pid = '0'"))
        {
            ...
            foreach (DataRow childmenu in menu_dt.Select(string.Format("pid = '{0}'", dr["id"].ToString())))
            {
        ...
        for (int nI = 0; nI < menubutton.Count; nI++)
        {
            ...
            if (menubutton[nI].SubButton.Count == 0)
            {
                if (menubutton[nI].Value.IndexOf("http://") == 0)
                    menuStr.Append("{\"type\":\"view\",\"name\":\"" + menubutton[nI].Name + "\",\"url\":\"" + menubutton[nI].Value + "\"}");
                else
                    menuStr.Append("{\"type\":\"click\",\"name\":\"" + menubutton[nI].Name + "\",\"key\":\"" + menubutton[nI].Value + "\"}");
            }
            else
            {
                ...
                for (int nJ = 0; nJ < menubutton[nI].SubButton.Count; nJ++)
                {
                    ...
                    if (menubutton[nI].SubButton[nJ].Value.IndexOf("http://") == 0)
                        menuStr.Append("{\"type\":\"view\",\"name\":\"" + menubutton[nI].SubButton[nJ].Name + "\",\"url\":\"" + menubutton[nI].SubButton[nJ].Value + "\"}");
                    else
                        menuStr.Append("{\"type\":\"click\",\"name\":\"" + menubutton[nI].SubButton[nJ].Name + "\",\"key\":\"" + menubutton[nI].SubButton[nJ].Value + "\"}");
