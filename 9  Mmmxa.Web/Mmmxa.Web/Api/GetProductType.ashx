<%@ WebHandler Language="C#" Class="GetProductType" %>
...
public class GetProductType : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        else if (context.Request.QueryString["isshow"] != null)
        {
            ...
            int i = shopNum1_ProductCategory_Action.UpdateIsshow(strreq.Split('-')[1].ToString(), strreq.Split('-')[0].ToString());
        ...
        else if (context.Request.QueryString["vd"] != null)
        {
            ...
            if (strreq.IndexOf(",") != -1)
            {
                ...
                {
                    shopNum1_ProductCategory_Action.UpdateOrderName(str[i].Split('|')[1].ToString(), str[i].Split('|')[2].ToString(), str[i].Split('|')[0].ToString());
                }
            }
            else
            {
                shopNum1_ProductCategory_Action.UpdateOrderName(strreq.Split('|')[1].ToString(), strreq.Split('|')[2].ToString(), strreq.Split('|')[0].ToString());
            }
        ...
        else if (context.Request.QueryString["savedata"] != null)
        {
            ...
            shopNum1_ProductCategory_Action.UpdateOrderName(strreq.Split('|')[0].ToString(), strreq.Split('|')[2].ToString(), strreq.Split('|')[1].ToString());
            context.Response.Write("ok");
        }
        else if (context.Request.QueryString["delectid"] != null)
        {
            ...
            DataTable dataTable = shopNum1_ProductCategory_Action.SearchtTypeId(Convert.ToInt32(strreq), 0);//获取分类表相关的分类编号
            ...
            if (shopNum1_ShopInfoList_Action.CheckType(strreq) == "")
            {
                ...
                else
                {
                    ...
                    {
                        bool bflag = true;
                        dt_three =shopNum1_ProductCategory_Action.SearchtTypeId(Convert.ToInt32(dataTable.Rows[i]["id"]), 0);
    ...
    {
        ShopNum1_SpecificationProudctCategory_Action SpecificationProudctCategoryAction = (ShopNum1_SpecificationProudctCategory_Action)LogicFactory.CreateShopNum1_SpecificationProudctCategory_Action();
