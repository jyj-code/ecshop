...
{
	[ParseChildren(true)]
	public class S_RepertoryAlertProduct : BaseShopWebControl
	{
		private string string_0 = "S_RepertoryAlertProduct.ascx";
		...
		public S_RepertoryAlertProduct()
		{
		...
		private string method_1()
		{
			...
			stringBuilder.Append(" and memloginid='" + this.MemLoginID + "' and issell=1 and isdeleted=0 and isaudit=1 and issaled=1 and RepertoryCount<=RepertoryAlertCount ");
			if (this.string_5 != "")
			{
				stringBuilder.Append(" and productseriescode like '%" + HttpUtility.HtmlDecode(this.string_5).Replace("%2f", "/") + "%' ");
			}
			if (this.string_1 != "")
			{
				stringBuilder.Append(" and name like '%" + HttpUtility.HtmlDecode(this.string_1).Replace(" ", "").Replace("%2f", "/").Trim() + "%' ");
			}
			if (this.string_2 != "")
			{
				stringBuilder.Append(" and productnum like '%" + HttpUtility.HtmlDecode(this.string_2).Replace(" ", "").Replace("%2f", "/").Trim() + "%' ");
			}
		...
		private void method_2()
		{
			...
			PageListBll pageListBll = new PageListBll("shop/ShopAdmin/S_SellProduct.aspx", true);
			this.htmlGenericControl_0.InnerHtml = pageListBll.GetPageListNew(pageList);
			S_RepertoryAlertProduct.dt_ShowProduct = shop_Product_Action.GetProductList(this.PageSize.ToString(), text, this.method_1(), "1");
			if (S_RepertoryAlertProduct.dt_ShowProduct.Rows.Count == 0)
			{
				S_RepertoryAlertProduct.dt_ShowProduct = null;
			}
		}
	}
}
