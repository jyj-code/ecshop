...
namespace ShopNum1.ShopAdminWebControl
{
	...
	{
		private string string_0 = "S_PaipaiImport.ascx";
		...
		protected bool ImportCSV(string strPath)
		{
			...
			try
			{
				...
				if (directories.Length == 1 && files.Length == 0)
				{
					...
					this.dataTable_0 = S_PaipaiImport.read_csv(filepath);
				...
				else
				{
					...
					while (num3 < this.dataTable_0.Rows.Count && num2 > 0)
					{
						try
						{
							...
							string arg_585_1 = "=\"\"[\\w](.*?)\"\"";
							if (S_PaipaiImport.matchEvaluator_0 == null)
							{
								S_PaipaiImport.matchEvaluator_0 = new MatchEvaluator(S_PaipaiImport.smethod_0);
							}
							arg_58A_0.Detail = Regex.Replace(arg_585_0, arg_585_1, S_PaipaiImport.matchEvaluator_0);
						...
						for (i = 0; i < list.Count; i++)
						{
							if (list[i].Length > 10)
							{
								if (i != 0)
								{
									...
									{
										if (list[0][j] != null && !(list[0][j] == string.Empty))
										{
											dataRow[j] = list[i][j];
											j++;
										}
