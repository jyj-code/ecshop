...
public class Main_Admin_ShopInfoList_Manage : PageBase, IRequiresSessionState
{
	...
	protected void ButtonOperate_Click(object sender, EventArgs e)
	{
		...
		if (text == "5" || text == "8")
		{
			...
			if (shopNum1_ShopInfoList_Action.UpdateShopState(this.CheckGuid.Value, "IsSystemRecommend", text) > 0)
			{
				this.method_5();
				this.MessageShow.ShowMessage("Audit2Yes");
				this.MessageShow.Visible = true;
		...
		if (text == "23" || text == "24")
		{
			...
			if (shopNum1_ShopInfoList_Action.UpdateShopState(this.CheckGuid.Value, "IsSystemHot", string_) > 0)
			{
				this.method_5();
				this.MessageShow.ShowMessage("Audit2Yes");
				this.MessageShow.Visible = true;
	...
	protected void ButtonDelete_Click(object sender, EventArgs e)
	{
		try
		{
			...
			else
			{
				...
				if (shopInfoByGuid.Rows.Count != 0)
				{
					...
					if (num > 0)
					{
						HttpCookie httpCookie = base.Request.Cookies["AdminLoginCookie"];
						if (httpCookie != null)
						{
							HttpCookie httpCookie2 = HttpSecureCookie.Decode(httpCookie);
							this.Session["AdminLoginCookie"] = httpCookie2.Values["LoginID"].ToString();
