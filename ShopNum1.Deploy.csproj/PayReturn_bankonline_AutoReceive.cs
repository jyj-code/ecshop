...
using System.Web.UI;
public class PayReturn_bankonline_AutoReceive : Page, IRequiresSessionState
{
			...
			if (this.v_pstatus.Equals("20"))
			{
				if (this.v_oid.IndexOf("C") == -1)
				{
					...
					if (num == 1 || num == 0)
					{
						...
						string guid = dataTable.Rows[0]["OrderNumber"].ToString();
						shopNum1_OrderInfo_Action.SetPaymentStatus2(guid, 1, 1, 0, DateTime.Now, Convert.ToDecimal(this.v_amount), Convert.ToDecimal(value));
				...
				else if (this.v_oid.IndexOf("C") != -1)
				{
					...
					string nameById = Common.GetNameById("advancepayment", "shopnum1_member", " and memloginId='" + this.remark2 + "'");
