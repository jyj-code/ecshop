...
public class Admin_ArticleCategory_Operate : PageBase, IRequiresSessionState
{
	...
	protected void ButtonConfirm_Click(object sender, EventArgs e)
	{
		...
		{
			int num = 0;
			string nameById = Common.GetNameById("CategoryLevel", "ShopNum1_ArticleCategory", "  and  ID='" + this.DropDownListFatherID.SelectedValue + "'");
			...
			if (num >= 2)
			{
				MessageBox.Show("不允许添加三级和三级以上的分类！");
			...
			else
			{
				...
				int num2 = shopNum1_ArticleCategory_Action.Add(shopNum1_ArticleCategory);
				if (num2 > 0)
				{
		...
		else if (this.ButtonConfirm.ToolTip == "Update")
		{
			...
			int num2 = shopNum1_ArticleCategory_Action2.Update(shopNum1_ArticleCategory);
			if (num2 > 0)
			{
