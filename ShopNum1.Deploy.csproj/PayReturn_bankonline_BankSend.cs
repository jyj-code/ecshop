...
using System.Web.UI;
public class PayReturn_bankonline_BankSend : Page, IRequiresSessionState
{
		...
		if (this.Page.Request.QueryString["ID"] != null)
		{
			...
			else
			{
				...
				else
				{
					...
					this.v_md5info = FormsAuthentication.HashPasswordForStoringInConfigFile(password, "md5").ToUpper();
					this.v_rcvname = HttpUtility.UrlEncode(dataTable.Rows[0]["Name"].ToString());
					this.v_rcvaddr = HttpUtility.UrlEncode(dataTable.Rows[0]["Address"].ToString());
					...
					this.v_ordername = HttpUtility.UrlEncode(dataTable.Rows[0]["Name"].ToString());
					this.v_orderaddr = HttpUtility.UrlEncode(dataTable.Rows[0]["Address"].ToString());
					...
					this.remark1 = HttpUtility.UrlEncode(dataTable.Rows[0]["ClientToSellerMsg"].ToString());
					this.remark2 = "";
				}
			}
		}
