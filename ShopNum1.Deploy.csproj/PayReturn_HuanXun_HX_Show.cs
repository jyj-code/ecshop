...
public class PayReturn_HuanXun_HX_Show : Page, IRequiresSessionState
{
		...
		if (flag)
		{
			...
			else if (text.IndexOf("C") != -1)
			{
				...
				if (dataTable.Rows[0]["OperateStatus"].ToString() == "0")
				{
					...
					this.AdvancePaymentModifyLog(1, Convert.ToDecimal(nameById), Convert.ToDecimal(text2), "环迅支付会员充值", this.method_0(), DateTime.Now.ToLocalTime(), 1);
			...
			else
			{
				...
				string text8 = dataTable2.Rows[0]["Guid"].ToString();
				...
				if (Convert.ToInt32(value2) == 0)
				{
					...
					shopNum1_OrderProduct_Action.UpdateStock(text8);
					ShopNum1_Member_Action shopNum1_Member_Action = (ShopNum1_Member_Action)LogicFactory.CreateShopNum1_Member_Action();
					shopNum1_Member_Action.UpdateCostMoney(memLoginID, Convert.ToDecimal(text2));
					string nameById = Common.GetNameById("advancepayment", "shopnum1_member", " and memloginId='" + this.method_0() + "'");
					this.AdvancePaymentModifyLog(3, Convert.ToDecimal(nameById), Convert.ToDecimal(Common.ReqStr("price")), "环迅支付会员购买商品", this.method_0(), DateTime.Now.ToLocalTime(), 0);
					this.OrderOperateLog("环迅支付会员购买商品", "买家已付款", "等待卖家发货", text8);
				}
			}
			base.Response.Write("ipscheckok");
		}
		else
		{
