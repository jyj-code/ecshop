...
using System.Web.UI;
public class PayReturn_ShengPay_SPSend : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		string text = ConfigurationManager.AppSettings["keyMer"].Trim();
		...
		string text2 = PayReturn_ShengPay_SPSend.byte2mac(signed);
		string text3 = text2.ToUpper();
		string value = base.Request.Form["TransAmount"].ToString();
		string text4 = base.Request.Form["OrderNo"].ToString();
		if (text3.Equals(base.Request.Form["SignMsg"].ToString().Trim()))
		{
			if (text4.IndexOf("C") != -1)
			{
				...
				DataTable dataTable = shopNum1_AdvancePaymentApplyLog_Action.SelectOperateMoney("", "1", "", "", text4, "", "");
				...
				if (dataTable.Rows[0]["OperateStatus"].ToString() == "0")
				{
					...
					this.AdvancePaymentModifyLog(1, Convert.ToDecimal(nameById), Convert.ToDecimal(value), "盛付通会员充值", this.method_0(), DateTime.Now.ToLocalTime(), 1);
					shopNum1_AdvancePaymentApplyLog_Action.ChangeApplyLogStatus(1, text4);
				}
			}
			else
			{
				ShopNum1_OrderInfo_Action shopNum1_OrderInfo_Action = (ShopNum1_OrderInfo_Action)LogicFactory.CreateShopNum1_OrderInfo_Action();
				DataTable dataTable2 = shopNum1_OrderInfo_Action.SearchOrderInfo(text4);
				string memLoginID = dataTable2.Rows[0]["MemLoginID"].ToString();
				string text5 = dataTable2.Rows[0]["Guid"].ToString();
				...
				{
					shopNum1_OrderInfo_Action.SetPaymentStatus2(text5, 1, 1, 0, DateTime.Now, Convert.ToDecimal(value), Convert.ToDecimal(value2));
					...
					shopNum1_Member_Action.UpdateCostMoney(memLoginID, Convert.ToDecimal(value));
					string nameById = Common.GetNameById("advancepayment", "shopnum1_member", " and memloginId='" + this.method_0() + "'");
					this.AdvancePaymentModifyLog(3, Convert.ToDecimal(nameById), Convert.ToDecimal(Common.ReqStr("price")), "盛付通会员购买商品", this.method_0(), DateTime.Now.ToLocalTime(), 0);
					this.OrderOperateLog("盛付通会员购买商品", "买家已付款", "等待卖家发货", text5);
				}
			}
