...
public class Admin_SellOrder_Report : PageBase, IRequiresSessionState
{
	...
	private void method_5()
	{
		...
		DataTable dataTable = shopNum1_Report_Action.SearchSellOrder(this.Page.Request.QueryString["pname"].ToString(), this.Page.Request.QueryString["sname"].ToString(), this.Page.Request.QueryString["DispatchTime1"].ToString(), this.Page.Request.QueryString["DispatchTime2"].ToString());
		...
		if (dataTable != null && dataTable.Rows.Count > 0)
		{
			for (int i = 0; i < dataTable.Rows.Count; i++)
			{
				...
				base.Response.Write("<td align=\"center\" width=\"500\" style=\"vnd.ms-excel.numberformat:@\"> {ProductName}</td>".Replace("{ProductName}", dataTable.Rows[i]["ProductName"].ToString()));
				base.Response.Write("<td align=\"center\" width=\"200\" style=\"vnd.ms-excel.numberformat:@\"> {ShopName}</td>".Replace("{ShopName}", dataTable.Rows[i]["ShopName"].ToString()));
				...
				base.Response.Write("<td align=\"center\" width=\"60\"  style=\"vnd.ms-excel.numberformat:@\"> {BuyPrice}</td>".Replace("{BuyPrice}", dataTable.Rows[i]["BuyPrice"].ToString()));
				base.Response.Write("<td align=\"center\" width=\"60\"  style=\"vnd.ms-excel.numberformat:@\"> {AveragePrice}</td>".Replace("{AveragePrice}", dataTable.Rows[i]["AveragePrice"].ToString()));
				base.Response.Write("<td align=\"center\" width=\"60\"  style=\"vnd.ms-excel.numberformat:@\"> {FeePay}</td>".Replace("{FeePay}", dataTable.Rows[i]["dispatchprice"].ToString()));
				base.Response.Write("</tr>");
			}
		}
