...
using tenpay;
public class tenpayAppreturn_url : Page, IRequiresSessionState
{
		...
		if (payResponseHandler.isTenpaySign())
		{
			...
			if ("0".Equals(parameter3))
			{
				try
				{
					...
					{
						decimal payMoney = Convert.ToDecimal(Common.ReqStr("total_fee")) / Convert.ToDecimal(100);
						...
						this.AdvancePaymentModifyLog(1, Convert.ToDecimal(nameById), payMoney, "财付通即时到账会员充值", this.method_0(), DateTime.Now.ToLocalTime(), 1);
