...
public class AgentAdmin_AdvertisementImg_Operate : PageBase, IRequiresSessionState
{
	...
	public void Add()
	{
		...
		try
		{
			num = this.defaultAdvertismentOperate_0.Add(title, imgsrc, href, width, height, pageName, string_, base.SubstationID);
		}
		catch (Exception)
		{
		}
	...
	public void Edit()
	{
		...
		int num = defaultAdvertismentOperate.Update(string_, title, imgsrc, linkUrl, height, width, pageName, string_2, base.SubstationID);
		if (num > 0)
		{
			base.Response.Redirect("AdvertisementImg_List.aspx");
		}
	...
	{
		DataRow dataRow = DefaultAdvertismentOperate.SelecByID(this.hiddenFieldGuid.Value.Replace("'", ""), base.SubstationID).Rows[0];
