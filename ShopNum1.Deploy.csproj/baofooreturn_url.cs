...
using System.Web.UI;
public class baofooreturn_url : Page, IRequiresSessionState
{
		...
		if (text8.ToLower() == Baofoo.Md5Encrypt(strToBeEncrypt).ToLower())
		{
			...
			if (text2.IndexOf('C') == -1)
			{
				...
				if (num == 1 || num == 0)
				{
					...
					string guid = dataTable.Rows[0]["TradeID"].ToString();
			...
			else
			{
				...
				{
					string nameById = Common.GetNameById("advancepayment", "shopnum1_member", " and memloginId='" + dataTable2.Rows[0]["MemLoginID"].ToString() + "'");
					this.AdvancePaymentModifyLog(1, Convert.ToDecimal(nameById), Convert.ToDecimal(((double)int.Parse(text5) * 0.01).ToString()), "宝付支付会员充值", dataTable2.Rows[0]["MemLoginID"].ToString(), DateTime.Now.ToLocalTime(), 1);
