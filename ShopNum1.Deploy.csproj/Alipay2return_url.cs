...
public class Alipay2return_url : Page, IRequiresSessionState
{
		...
		if (requestGet.Count > 0)
		{
			...
			if (responseTxt == "true" && a == mysign)
			{
				...
				if (base.Request.QueryString["trade_status"] == "WAIT_SELLER_SEND_GOODS" || base.Request.QueryString["trade_status"] == "TRADE_FINISHED")
				{
					...
					string text13 = dataTable.Rows[0]["Guid"].ToString();
					string value = dataTable.Rows[0]["ShouldPayPrice"].ToString();
					dataTable.Rows[0]["OrderNumber"].ToString();
					shopNum1_OrderInfo_Action.SetPaymentStatus2(text13, 1, 1, 0, DateTime.Now, Convert.ToDecimal(text3), Convert.ToDecimal(value));
					this.Page.Response.Redirect("~/index.aspx?MemberUrl=" + HttpUtility.UrlEncode("OrderDetail.aspx?guid=" + text13));
				}
				else
				{
