...
public class AgentAdmin_SeeBuyRate_Report : PageBase, IRequiresSessionState
{
	...
	private int method_5()
	{
		...
		if (dataTable != null && dataTable.Rows.Count > 0)
		{
			...
			{
				base.Response.Write("<tr>");
				base.Response.Write("<td align=\"left\" width=\"100\" style=\"vnd.ms-excel.numberformat:@\"> {Name}</td>".Replace("{Name}", dataTable.Rows[i]["Name"].ToString()));
				base.Response.Write("<td align=\"left\" width=\"200\" style=\"vnd.ms-excel.numberformat:@\"> {Ptype}</td>".Replace("{Ptype}", dataTable.Rows[i]["productcategoryname"].ToString()));
				base.Response.Write("<td align=\"left\" width=\"100\" style=\"vnd.ms-excel.numberformat:@\"> {Btype}</td>".Replace("{Btype}", dataTable.Rows[i]["brandname"].ToString()));
				base.Response.Write("<td align=\"left\" width=\"50\"  style=\"vnd.ms-excel.numberformat:@\"> {ClickCount}</td>".Replace("{ClickCount}", dataTable.Rows[i]["ClickCount"].ToString()));
				base.Response.Write("<td align=\"left\" width=\"50\"  style=\"vnd.ms-excel.numberformat:@\"> {SaleNumber}</td>".Replace("{SaleNumber}", dataTable.Rows[i]["SaleNumber"].ToString()));
				base.Response.Write("<td align=\"left\" width=\"50\"  style=\"vnd.ms-excel.numberformat:@\"> {BuyRate}</td>".Replace("{BuyRate}", dataTable.Rows[i]["BuyRate"].ToString()));
				base.Response.Write("</tr>");
			}
			base.Response.Write("</table> ");
