...
namespace ShopNum1.WebControl
{
	...
	{
		private string string_0 = "Search_productlist.ascx";
		...
		private string string_1 = GetPageName.RetDomainUrl("Search_productlist");
		...
		protected override void InitializeSkin(System.Web.UI.Control skin)
		{
			...
			this.string_5 = ((ShopNum1.Common.Common.ReqStr("ordername") == "") ? "salenumber" : ShopNum1.Common.Common.ReqStr("ordername"));
			this.string_4 = ((ShopNum1.Common.Common.ReqStr("sort") == "") ? "desc" : "asc");
		...
		{
			this.string_3 = ((this.textBox_3.Text.Trim().Replace("'", "") == string.Empty) ? "-1" : this.textBox_3.Text.Trim().Replace("'", ""));
			string url = GetPageName.AgentRetUrl("Search_productlist", this.string_3, "search");
		...
		{
			string text = AddressCode.ToString();
			...
			if (text.Length > 0)
			{
				...
				DataTable regionByCode = shopNum1_Region_Action.GetRegionByCode(code);
				if (regionByCode.Rows.Count > 0 && regionByCode != null)
				{
					result = regionByCode.Rows[0]["Name"].ToString();
				}
				else
				{
					result = "中国";
