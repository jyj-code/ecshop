...
namespace ShopNum1.ShopWebControl
{
	...
	{
		private string string_0 = "ProductIsSpellMeta.ascx";
		...
		protected override void InitializeSkin(Control skin)
		{
			...
			string memloginId = (this.Page.Request.QueryString["ShopID"].ToString() == null) ? "0" : this.Page.Request.QueryString["ShopID"].ToString();
			...
			DataTable spellInfoMeta = shop_Product_Action.GetSpellInfoMeta(this.MemLoginID, guid);
			if (spellInfoMeta.Rows.Count > 0)
			{
				if (!string.IsNullOrEmpty(spellInfoMeta.Rows[0]["Title"].ToString()))
				{
					this.literal_0.Text = "\n<title>" + spellInfoMeta.Rows[0]["Title"].ToString() + "</title>\n";
				}
				else
				{
					this.literal_0.Text = "\n<title>" + spellInfoMeta.Rows[0]["Title"].ToString() + "</title>\n";
				...
				{
					this.literal_1.Text = "<meta name=\"keywords\" content=\"" + spellInfoMeta.Rows[0]["KeyWords"].ToString() + "\">\n";
				}
				else
				{
					this.literal_1.Text = "<meta name=\"keywords\" content=\"" + spellInfoMeta.Rows[0]["Title"].ToString() + "\">\n";
				}
				if (!string.IsNullOrEmpty(spellInfoMeta.Rows[0]["Description"].ToString()))
				{
					this.literal_2.Text = "<meta name=\"description\" content=\"" + spellInfoMeta.Rows[0]["Description"].ToString() + "\">\n";
				}
				else
				{
					this.literal_2.Text = "<meta name=\"description\" content=\"" + spellInfoMeta.Rows[0]["Title"].ToString() + "\">\n";
				}
			}
		}
	}
