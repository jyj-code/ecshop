...
{
	[ParseChildren(true)]
	public class M_ArticleComment : BaseMemberWebControl
	{
		private string string_0 = "M_ArticleComment.ascx";
		...
		public static string GetTitle(string guid)
		{
			...
			DataTable titleByGuid = shopNum1_ArticleCheck_Action.GetTitleByGuid(guid);
		...
		private void method_0()
		{
			...
			{
				text = text + "  AND  SendTime>'" + this.textBox_0.Text.Trim() + "'   ";
			}
			if (!string.IsNullOrEmpty(this.textBox_1.Text.Trim()))
			{
				text = text + "   AND  SendTime<'" + this.textBox_1.Text.Trim() + "'  ";
			}
			...
			DataTable dataTable = shopNum1_ArticleCheck_Action.Select_List(commonPageModel);
			...
			DataTable dataTable2 = shopNum1_ArticleCheck_Action.Select_List(commonPageModel);
